...
- An actual-pull recheck at `2026-04-01 14:08 JST` after `mind-upload@ccef40f`, `EEG-DATA@deefd5f95c6`, and `auto-research-funds@f5f76c485` still leaves the public batch and the solve-first order unchanged.
- The newest pulled deltas still deepen route support rather than the current claim surface: `mind-upload` already keeps the route-hold wording public-safe, `EEG-DATA` now reports `merged_unique=30869`, `high_confidence=28832`, and `access_counts_high.immediate=24482`, and its new experiment-metadata report now covers `18,312 / 28,832` rows with at least one experiment-facing field (`63.51%` coverage).
- The route-safe access reading also stays narrow in one specific way: the refreshed local `EEG-DATA` catalog still shows `D03 / D02` as catalog-side immediate rows and `D01` as a mixed immediate/check-needed pair, but the live-access read remains stricter, so `D10` still remains the only clear public-open primary anchor while `D03 / D02 / D01` still stay gated-support anchors in public wording.
...
| `U13-5` | `Perception-to-recall transfer-gap benchmark for mimic separation` | `D03 Multimodal Fusion EEG + Eye-Tracking` still fixes the intended paired EEG + gaze target, but the current IEEE DataPort page is subscription-gated, so this row stays only as a gated support route rather than an immediate public route. | `Kura Fund`, then `Nakatani Foundation graduate scholarship` | A shared decoder route can support a bounded transfer-gap note, but it does not by itself justify a stronger preserved-mechanism reading. |
...
| `U14-4` | `Card-integrated reproducibility contract for the current public batch` | `D01 Acquisition delay of wireless EEG` is still the intended latency-first reference, but the current IEEE DataPort page is subscription-gated, so the row stays only as a gated support route for disclosure-first wording. | `Kura Fund`; `Nakatani Foundation research grant` as next-cycle watch | Filled card fields can improve disclosure discipline, but they do not by themselves demonstrate field-wide reproducibility, deployment readiness, or standards adoption. |
| `U14-2` | `Fixed-split rerun contract for exploration versus confirmation` | The same `D01` route still marks the intended latency-first split boundary, but public wording now keeps it only as a subscription-gated support anchor until a public-open rerun family is stronger. | `Kura Fund`; `Nakatani Foundation research grant` as next-cycle watch | A fixed split and negative-case trail can bound rerun discipline, but they do not license confirmatory wording for exploratory outputs or broad rerun-robustness claims. |
...
| `U13-5` | `D03` still fixes the intended paired EEG + gaze route, but the current IEEE DataPort page is subscription-gated, so the row now stays only as a gated support route rather than an immediate public route. | A perception-to-recall degradation note and a transfer-gap table. | Richer recall-label validity, a speech-richer adjunct benchmark, and any stronger mechanism-preservation wording. | `Perception-to-recall transfer-gap benchmark for mimic separation` |
...
- Why `D03` remains only a gated support route: the intended paired EEG + eye-tracking target is still useful for the row, but the current IEEE DataPort page is subscription-gated, so public wording must stay narrower than an immediate-public-anchor reading.
