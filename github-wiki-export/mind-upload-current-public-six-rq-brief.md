...
- `D10 3M-CPSEED` and `D02 CSTE` remain the stronger primary anchors.
- `D03 Multimodal Fusion EEG + Eye-Tracking` and `D01 Acquisition delay of wireless EEG` remain bounded-support anchors with lower claim ceilings.
...
| `U13-5` | `Perception-to-recall transfer-gap benchmark for mimic separation` | `D03 Multimodal Fusion EEG + Eye-Tracking` still gives an immediate paired EEG + gaze route, which is enough to freeze where performance branches between perception and recall under a shared decoder. | `Kura Fund`, then `Nakatani Foundation graduate scholarship` | A shared decoder route can support a bounded transfer-gap note, but it does not by itself justify a stronger preserved-mechanism reading. |
| `U0-2` | `Temporal-validity-aware identity drift audit with offset, jitter, and state-feature collapse` | `D02 CSTE` still remains cross-session and cross-task, immediately accessible, and explicitly biometric, so timing drift can be bounded before identity-like wording absorbs measurement error. | `Nakatani Foundation graduate scholarship`, then `Kashinome Scholarship` | This can audit timing and state-feature failure, but it does not settle legal, philosophical, or normative identity continuity. |
| `U0-3` | `Task-specific threshold stability and overfit-exclusion benchmark for identity drift` | The same `D02` anchor keeps threshold stability on the same cross-session / cross-task floor as `U0-2`, which is the minimum condition for interpreting threshold movement at all. | `Nakatani Foundation graduate scholarship`, then `Kashinome Scholarship` | A stable threshold can still remain only a bounded benchmark artifact, not a sufficient condition for personhood continuity or branch-identity resolution. |
| `U14-4` | `Card-integrated reproducibility contract for the current public batch` | `D01 Acquisition delay of wireless EEG` is narrow, but it is still enough to force latency and measurement fields into concrete Model Card / Dataset Card examples rather than leaving disclosure abstract. | `Kura Fund`; `Nakatani Foundation research grant` as next-cycle watch | Filled card fields can improve disclosure discipline, but they do not by themselves demonstrate field-wide reproducibility, deployment readiness, or standards adoption. |
| `U14-2` | `Fixed-split rerun contract for exploration versus confirmation` | The same `D01` route is enough to make the exploration / confirmation split operational and measurable before a broader benchmark family is added. | `Kura Fund`; `Nakatani Foundation research grant` as next-cycle watch | A fixed split and negative-case trail can bound rerun discipline, but they do not license confirmatory wording for exploratory outputs or broad rerun-robustness claims. |
...
| `U13-5` | Freeze where performance branches between perception and recall under a shared decoder. | `D03 Multimodal Fusion EEG + Eye-Tracking` (`10.21227/6106-6120`, immediate) | Perception-to-recall degradation note and transfer-gap table | `Kura Fund`, then `Nakatani Foundation graduate scholarship` | Any stronger mechanism-preservation reading |
| `U0-2` | Fix the timing budget before identity-like wording absorbs measurement error. | `D02 CSTE` (`10.21227/j162-nh61`, immediate) | Offset/jitter distribution and state-feature collapse audit | `Nakatani Foundation graduate scholarship`, then `Kashinome Scholarship` | Legal, philosophical, or normative identity continuity |
| `U0-3` | Fix threshold stability only after the timing floor is bounded. | `D02 CSTE` (`10.21227/j162-nh61`, immediate) | Threshold-stability note and overfit-exclusion rule | `Nakatani Foundation graduate scholarship`, then `Kashinome Scholarship` | Treating a stable threshold as sufficient for personhood continuity |
| `U14-4` | Turn card fields into a concrete rerun checklist. | `D01 Acquisition delay of wireless EEG` (`10.21227/dv1p-vq18`, immediate) | Filled Model Card / Dataset Card examples | `Kura Fund`; `Nakatani Foundation research grant` as next-cycle watch | Field-wide reproducibility, deployment readiness, or standards adoption |
| `U14-2` | Make the exploration / confirmation split operational. | `D01 Acquisition delay of wireless EEG` (`10.21227/dv1p-vq18`, immediate) | Fixed-split rerun contract and negative-case trail | `Kura Fund`; `Nakatani Foundation research grant` as next-cycle watch | Presenting exploratory outputs as confirmatory evidence |
...
| `U13-5` | `D03` still keeps an immediate DOI route with paired EEG, gaze, and related physiological streams, which is enough to keep the branch point concrete. | A perception-to-recall degradation note and a transfer-gap table. | Richer recall-label validity, a speech-richer adjunct benchmark, and any stronger mechanism-preservation wording. | `Perception-to-recall transfer-gap benchmark for mimic separation` |
| `U0-2` | `D02` still exposes `10 participants`, cross-session and cross-task EEG biometrics, and a biometric benchmark framing on an immediate route. | An `offset / jitter` distribution plus a `state-feature collapse` audit. | Longer-horizon timing logs, larger cohorts, and any legal or philosophical continuity interpretation. | `Temporal-validity-aware identity-drift audit` |
| `U0-3` | The same `D02` route still lets threshold selection stay on the same cross-session / cross-task floor as `U0-2`. | A threshold-stability note and an overfit-exclusion rule. | A session-external rerun pack, cohort extension, and any personhood-continuity reading. | `Task-specific identity-threshold stability study` |
| `U14-4` | `D01` still gives an immediate latency-first wireless EEG route in a wearable / ambulatory setting, which is enough for disclosure-first rerun discipline. | Filled Model Card / Dataset Card examples for the current batch. | Richer rerun benchmarks, leak-audit exemplars, and any field-wide reproducibility or standards-adoption wording. | `Disclosure-first reproducibility contract package` |
| `U14-2` | The same `D01` latency-first route is still enough to define a fixed split and rerun boundary even before a broader benchmark family exists. | A fixed-split rerun contract and a negative-case trail. | Split-violation examples across richer public benchmarks and any stronger confirmatory-readiness claim. | `Exploration-confirmation split contract for reruns` |
...
- Why `D03` remains acceptable but bounded: the current mirror still preserves an immediate DOI route with paired EEG and eye-tracking plus related physiological streams, which is enough to keep the branch-point reading concrete.
...
- Why `D02` remains the cleanest floor: the dataset is explicitly cross-session and cross-task, framed as EEG biometrics, immediately accessible, and the current mirror still keeps it tied to `10 participants`.
...
- Why the same `D02` anchor stays correct: threshold stability remains meaningful only when it sits on the same cross-session / cross-task identity-drift floor as `U0-2`, rather than on a single-session score.
...
- Why `D01` is still useful only as a bounded entry point: a wireless-delay dataset is narrow, but it is enough to force latency and measurement fields into a real Model Card / Dataset Card example without claiming field-wide standardization.
...
- Why the same `D01` anchor still works: the latency-first measurement route is enough to freeze a fixed split plus rerun boundary, even before a richer benchmark family is added.
