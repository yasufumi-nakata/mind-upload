...
- 更新日: 2026-03-25 / 位置づけ: RQ-by-RQ Deep Dive
...
## 今回の深掘り実行パック（2026-03-25 04:02 JST, 2週間で初回結果まで）
...
|---|---|---|---|---|---|
| U0-3 | 同一性閾値の探索は過学習除外規則を最短で固定でき、U0全体の判定境界を安定化できる。 | `タスク別閾値安定性` `セッション外AUC` `過学習除外率` | セッション外で閾値が収束せず、除外規則がfoldごとに逆転する。 | Threshold search notebook and selection rationale table | `G2 / G3` |
| U1-1 | 逆問題の不良設定性は基礎手法の根幹で、事前分布比較を先に固定するとU1系全体の再現性が上がる。 | `逆解誤差` `事前分布間の順位一致率` `不確実性被覆率` | 事前分布ごとの順位が再現せず、誤差優位が固定できない。 | Inverse solution comparison table and uncertainty distribution chart | `G1 / G4` |
| U8-1 | 閉ループ遅延許容域は運用上の停止規則へ直結するため、短期で安全域を定量化できる。 | `安定率` `停止介入率` `回復時間` | 遅延増加時に安全域境界が再現せず、停止規則が過検知/過少検知に偏る。 | Closed loop safety KPI dashboard | `G2 / G5` |
| U11-2 | 指標計算量制約を先に固定すると、理論比較の過大主張を抑えつつ応募可能な形に落とせる。 | `計算予算内完走率` `順位保存率` `境界症例での符号一致率` | 予算制約下で順位が不安定化し、理論比較結果が条件依存で反転する。 | Theoretical comparison I/O specifications and calculation budget | `G2 / G4` |
| U12-2 | 同意継承の分岐点は制度接続の中核で、監査ログ項目を先に固定するとC判定RQの提出品質が上がる。 | `分岐時同意整合率` `権限伝播遅延` `証跡欠損率` | 分岐ごとに同意状態が追跡不能になり、監査鎖が一意対応しない。 | System audit requirements table (technical log compatible) | `G2 / G6` |
| U14-1 | 固定粒度の要求水準は追試成否を直接左右し、最小追試パックの閾値設定に実務価値が高い。 | `追試成功率` `環境固定差分の感度` `実行コスト` | 固定粒度を上げても追試成功率が改善せず、コスト増分のみが残る。 | Supplementary test operation report (including negative cases) | `G1 / G3` |
