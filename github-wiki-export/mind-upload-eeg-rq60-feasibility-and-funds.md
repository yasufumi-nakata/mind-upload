...
| G4 | 成茂神経科学研究助成基金 (`Drbm6vBRDJkn0NGJ`) | 2026-11-18〜2027-02-17（前年実績ベース） | 神経科学中核の `B` 判定RQ（U1/U4/U13）を、介入/失敗条件付きで出す。 |
...
| U4-3<br>active inferenceやDCMの理論予測を、反事実評価にどう接続するか。 | B（部分検証（外部データ併用で成立）） | ID 676でDCM/active inference由来の予測と実測差の比較タスクを設計する。 | 研究奨励金（反事実評価） | ID 19, 56, 1839 |
| U4-4<br>同値判定の失敗条件（falsification）をどの水準で宣言するか。 | B（部分検証（外部データ併用で成立）） | ID 19/56で「失敗宣言閾値」を先に固定し、再試験可能な反証設計にする。 | 共同研究助成（反証設計） | ID 19, 56, 1839 |

## U7 リサーチクエスチョン（RQ単位）

| RQ | EEG-DATAで解けるか | 深掘り検証設計（1問ずつ） | auto-research-funds向け応募テーマ | 推奨EEG-DATA |
|---|---|---|---|---|
| U7-1<br>BIDS拡張で同期・QC・刺激ログをどこまで必須化するか。 | A（直接検証可能） | ID 676/783で同期・QC・刺激ログをBIDS監査項目として固定する。 | 研究助成（マルチモーダル標準化） | ID 676, 783, 4878 |
...
| U7-5<br>前処理差分をCIで自動比較する場合、どの再現率低下をリリースブロック閾値にするか。 | A（直接検証可能） | ID 676で前処理CIを回し、再現率低下のブロック閾値を決める。 | 研究助成（CI再現性） | ID 676, 783, 4878 |
...
| RQ | EEG-DATAで解けるか | 深掘り検証設計（1問ずつ） | auto-research-funds向け応募テーマ | 推奨EEG-DATA |
|---|---|---|---|---|
| U8-1<br>閉ループBCIの遅延許容域を制御理論的にどう同定するか。 | B（部分検証（外部データ併用で成立）） | ID 6/1972で遅延・位相ずれと制御性能の関係を推定し、安全域を定義する。 | 若手研究助成（閉ループ安定化） | ID 6, 1972, 2412, 696 |
...
| RQ | EEG-DATAで解けるか | 深掘り検証設計（1問ずつ） | auto-research-funds向け応募テーマ | 推奨EEG-DATA |
|---|---|---|---|---|
| U11-1<br>理論間で比較可能な入出力仕様をどう定義するか。 | B（部分検証（外部データ併用で成立）） | ID 842/859で状態遷移時の指標順位保存性（PCI近似など）を比較する。 | 研究助成（意識指標比較） | ID 735, 842, 859 |
| U11-2<br>PCIやIIT近似計算の計算量制約をどう扱うか。 | C（EEG-DATA単独では不可（制度・他モダリティ必須）） | IIT厳密計算の計算量問題はEEG-DATAだけでは解けないため、近似法比較に限定する。 | 理論研究奨励（IIT近似） | ID 735, 842, 859 |
...
| U13-5<br>視覚知覚と視覚想起で同一デコーダを使ったとき、意味復元精度の劣化パターンはどこで分岐するか。 | B（部分検証（外部データ併用で成立）） | ID 509で知覚条件と想起条件を分け、劣化分岐点を推定する。 | 博士課程研究助成（知覚/想起分岐） | ID 509, 13, 65 |
...
| U14-5<br>再現失敗ケースを否定例レジストリとして公開し、再試行サイクルをどう運用するか。 | B（部分検証（外部データ併用で成立）） | 再現失敗レジストリはEEG-DATA単独で運用可能だが、公開同意フローの追加が必要。 | 研究奨励金（否定例レジストリ） | ID 6, 56, 719 |
