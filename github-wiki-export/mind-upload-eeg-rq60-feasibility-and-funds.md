...
- 更新日: 2026-03-17 / 位置づけ: RQ-by-RQ Deep Dive
...
## 2026-03-17 22:02 JST 1件深掘り追記（U8-2）

### U8-2（オンライン較正と概念ドリフト対策）の提出固定

- 第一応募先: `G2`（`GR-2026-014`）
- 予備応募先: `G5`（`871pw3rLjNPKgqA0`）
- 提出テーマ: `オンライン較正で概念ドリフトを抑制する閉ループEEG運用基盤`

### 使うEEG-DATAの種別（「何のやつか」）

| 役割 | EEG-DATA | 種別（刺激/実験条件） | このRQでの使い方 |
|---|---|---|---|
| 主 | `ID 6` (`10.6084/m9.figshare.28844471.v1`) | 視覚・運動課題 / `SSVEP`・`運動イメージ` | 再較正間隔ごとの安定率比較 |
| 主 | `ID 2412` (`10.6084/m9.figshare.30822632.v1`) | 運動課題 / `ERP`・`運動イメージ` | セッション間ドリフト耐性の確認 |
| 補助 | `ID 1972` (`10.5281/zenodo.7799759`) | 睡眠 / `睡眠` | 状態遷移を跨ぐ較正劣化の確認 |
| 補助 | `ID 696` (`10.6084/m9.figshare.31403511`) | 視覚 / `不明` | 日内・日間ドリフトの追試 |

### U8-2の合否ゲート（今回固定）

- `安定率` が事前基準を満たし、かつ `回復時間(s)` が許容上限以内であること。
- 未達の場合は `B` 判定のまま、外部要件（長期連続運用インフラ）を申請本文で明示すること。

## 2026-03-17 22:02 JST 再検証ログ（本run / RQごと深掘り継続）
