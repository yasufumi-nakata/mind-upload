...
> 前処理はクリーンアップではありません。それは請求契約の一部です
>
> この学習ページは GitHub Wiki 用に生成されています。公開ポータルは [mind-upload.com](https://mind-upload.com) で管理しています。

- Updated: 2026-04-04 / Role: 技術/実践ガイド (録画フレーム契約を明確にするために更新)

## このページの役割
このページでは、EEG 前処理と QC を、最終的な表面上のクリーンアップ ステップとしてではなく、どの信号が引き続き使用可能か、どの派生ブランチが再利用可能のままか、どのクレームを停止する必要があるかを監査するものとして扱います。

## 正確性に関する注記
このページでは、1 つのユニバーサル パイプラインを規定するものではありません。一次文献および公式仕様書が現在サポートしている最小限の開示および停止ルールを抽出します。
...
## 関連 Wiki ページ
- [Wiki: 脳波の基礎](https://github.com/yasufumi-nakata/mind-upload/wiki/eeg-basics) - 信号自体の性質から戻りたい場合は、ここをクリックしてください。
- [Wiki: イベント同期と監視ログ](https://github.com/yasufumi-nakata/mind-upload/wiki/event-sync-and-measurement-logs) - 前処理前に必要な時刻同期、イベント、不良セグメントの記録を補足します。
- [Wiki: データセットの分割と漏洩](https://github.com/yasufumi-nakata/mind-upload/wiki/dataset-splits-and-leakage) - 分割ユニットとホールドアウトの祖先自体を明確にする必要がある場合にこれを使用します。
- [Wiki: 不確実性、校正、棄権](https://github.com/yasufumi-nakata/mind-upload/wiki/uncertainty-confidence-and-abstention) - これは、前処理の違いを使用した推定幅と棄権の概念を補足します。
- [Wiki: 標準/場所/バリデータ/ベンチマーク](https://github.com/yasufumi-nakata/mind-upload/wiki/standards-repositories-validators-and-benchmarks) - BIDS、デリバティブ、パブリック バージョン、ローダー、ベンチマーク間の役割の違いを補います。
...
<h2>1。レポートフロアはアルゴリズム名</h2>ではなくメタデータです
<p>
...
<h2>2。前処理がデータから学習する場合、それはトレーニング Split</h2> 内に属します。
<p>
...
<h2>3。クリーンな EEG は、派生系統と祖先が明示的に残っている場合にのみ再利用可能</h2>
<p>
...
<h2>4。参照メソッドの実装上の小さな違いはありません。観測モデル</h2>の一部です
<p>
...
<h2>5。ハーモナイゼーションはレコーディング フレームの契約であり、バックグラウンド クリーンアップではありません</h2>
<p>
...
<h2>6。フィルターは「通過帯域」だけでなく歪み設計もできる</h2>
<p>
...
<h2>7。アーティファクトの抑制は必ずしも改善になるわけではない</h2>
<p>
...
<h2>8。ショートカットとフィンガープリントの監査は分類子</h2>の前に開始されます
<p>
...
<h2>10。高ベータ/ガンマは筋電検査</h2>なしでは強く書き込まれません
<p>
