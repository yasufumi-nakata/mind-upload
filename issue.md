...
<p>
The active public deepening batch is <code>U0-1</code>, <code>U1-2</code>, <code>U4-4</code>, <code>U7-1</code>, <code>U8-4</code>, and <code>U11-1</code>. These are not solved questions. They are the current bounded work packages where one claim, one dataset anchor, one KPI bundle, and one explicit external-dependency boundary are being fixed first. As of March 29, 2026, the same public batch remains active because the conservative anchor set <code>D02/D08/D05/D11/D20/D14</code> is now aligned across the RQ-by-RQ deep dossiers, the EEG feasibility page, and the current funding shortlist; this is a synchronization update, not a solved-claim update. The public batch is changed only after those route-setting pages agree on the same bounded route. For the exact <code>Dxx + DOI</code> anchors, first-pass KPIs, and the external dependencies kept outside the EEG claim, start with the <a href="wiki/mind-upload-rq60-deep-focus-notes.html">deep focus notes</a> and the <a href="wiki/mind-upload-eeg-rq60-feasibility-and-funds.html">RQ60 EEG feasibility page</a>.
</p>
</div>
<div class="note-box">
