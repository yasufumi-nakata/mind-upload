...
- 根拠:
  ...
  - Dürst et al. (2024) は bouton potency が主として vesicular release probability に依存し、releasable vesicle 数の寄与は高 release state で初めて強く出ることを示しました。
...
- 根拠:
  ...
  - Holler et al. (2021)、Dürst et al. (2024)、Vardalaki et al. (2022) を合わせると、synaptic-state line で直接強いのは依然として local paired physiology や state manipulation に近いルートです。
- 修正:
  - MICrONS は `same-brain structure-function link / local functional twin` を押し上げると書き換え、`current synaptic efficacy` 行そのものは Holler / Dürst / Alle 側へ寄せました。
...
- 根拠:
  - Alle et al. (2024) は human neocortical tissue で、sleep-like membrane potential state sequence が EPSP 振幅と synaptic consolidation を短時間で変えることを示しました。
- 批判:
...
- `wiki/connectome-is-not-enough.md`
  ...
  - 「シナプスは binary edge ではありません」を Holler / Dürst / Matsuzaki / Vardalaki / Alle を用いた構造対状態の切り分けへ改稿
  - site rules に `PSD / spine / synapse count を current weight と書かない` ルールを追加
  - 参考文献へ Dürst (2024), Alle (2024) を追加
- `wiki/measurement-stack-and-claim-ceiling.md`
  ...
  - 短い結論の対応表を `MICrONS = structure-function link`、`Holler / Dürst / Alle = local synaptic-state line` へ修正
  - site rules に `EM / PSD / spine / same-brain connectomics を current weight と書かない` を追加
  - 参考文献へ Holler (2021), Dürst (2024), Matsuzaki (2004), Vardalaki (2022), Alle (2024) を追加
- `verification.md`
  - `last_updated` を `2026-03-17` に更新
  ...
  - 参考文献へ Holler (2021), Matsuzaki (2004), Vardalaki (2022), Dürst (2024), Alle (2024) を追加
...
2. Dürst CD, Boele H-J, Schonewille M, Hoebeek FE. Number of releasable vesicles does not limit short-term plasticity at hippocampal synapses with low release probability. *Nature Communications*. 2024;15:6427.
   - https://doi.org/10.1038/s41467-024-50549-7
...
5. Alle H, et al. Membrane potential states gate synaptic consolidation in human neocortical tissue. *Nature Communications*. 2024;15:7705.
