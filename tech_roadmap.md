...
<p><strong>2026-03 再定義：</strong>神経修飾の観測は少なくとも <strong>(a) pupil / HRV などの mixed arousal proxy</strong>、<strong>(b) axon activity</strong>、<strong>(c) local transmitter sensor</strong>、<strong>(d) receptor atlas / pharmacology</strong> の 4 段に分けて読む必要があります。後段ほど chemical / receptor specificity は上がりますが、coverage は狭くなりやすく、逆に前段は広域 covariate として有用でも transmitter ground truth にはなりません。</p>
<p><strong>暫定方針：</strong>神経修飾は「1つの気分スカラー」ではなく、<strong>compartment</strong>、<strong>spatial scope</strong>、<strong>時間スケール</strong>、<strong>受容体依存性</strong>を持つ潜在状態として扱います。人データでは瞳孔径や HRV を共変量や層別化に使えても、それだけで脳幹アミン系を同定したとは言いません。</p>
<p><strong>次に必要：</strong>動物や侵襲系の transmitter sensor / receptor physiology / pharmacology / perturbation で proxy を較正し、ヒト側では不確実性つきの状態推定と棄権条件を公開します。背景は <a href="wiki/measurement-stack-and-claim-ceiling.html#neuromodulatory-proxy-ladder">Wiki: 計測スタックごとの observability と claim ceiling</a> と <a href="wiki/connectome-is-not-enough.html">Wiki: 配線図だけでは足りない理由</a> を参照してください。</p>
</div>
</details>
</div>
