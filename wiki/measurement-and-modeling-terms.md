---
layout: default
title: 'Wiki: 測定からモデリングまでの用語ガイド'
description: EEG、QC、BIDS、逆問題、ESI、DCM、SCM、ルートカード、コンパニオンカードを観察から検証、監査項目までの流れと関連付けて解説します。
article_type: ウィキ
subtitle: 単語を点で覚えるのではなく、流れでつなげて覚えましょう。
author: マインドアップロード研究プロジェクト
last_updated: '2026-04-04'
note: 学習ガイド (2026 年 4 月 4 日のコンパニオン カード分割で更新)
audience: 測定語とモデル語が混在している人や、用語集を流れで理解したい人。
reading_time: 10～15分
page_intro: このページは、EEGなどの計測用語、ESIやDCMなどのモデル用語、BIDSやQCなどの運用用語を「観察→整理→推定→検証」の流れで結び付けて理解するためのWikiです。個々の単語を暗記するのではなく、まずどの単語を使用するか、どこで議論をやめるべきかを区別することを目的としています。
accuracy_note: ここで示したフローは、理解できるように整理したものです。実際の研究では行きつ戻りつや例外はありますが、観察と推定、推定と検証を混同しないことが重要です。
page_highlights:
- EEG、QC、BIDS、ESI、DCM、SCM を 1 つのフローで理解できます。
- 監査項目を用いて、観察だけでは再構築にならない理由を切り分けます。
- 逆問題の主張は現在、磁場形成の可視性、フォワードモデルまたは伝導率負荷、ソルバーファミリーの不確実性、および検証クラスに分割されているため、「より良い ESI」は 1 つの進捗バーとして解釈されません。
- ESI、DCM、および SCM は同じように失敗するわけではありません。それぞれに異なる停止ルールが必要です。
- DCM も 1 つの進行状況バーではありません。候補モデルの比較、スケーラブルな検索、処理感度、および信頼性ウィンドウは、異なる監査軸です。
- クレームが複数の行またはステージにまたがる場合、単一のルート カードでは不十分です。 Fusion Card、Human Proxy Composition Card、および State-Continuity Bridge Card は、1
  つの汎用メタデータ バケットではなく、コンパニオン カードとして分離されるようになりました。
- より多くのモダリティ、同一脳連携、同一主題言語、およびコネクトーム制約はルート カードをバイパスしません。融合、人間とプロキシの構成、状態の継続性、識別可能性は個別の監査のままです。
- より強力な設計とは、単に「データが増える」だけではありません。このルートでは、どの識別可能性の目的が次の条件を選択したか、省略されたメカニズムのエラーが公開されたかどうか、およびどのような最小十分性の停止ルールで十分だったかも尋ねられます。
- 作業順序と証拠の強さに応じて、用語集内の定義を並べ替えます。
known_points:
...
- label: 'Wiki: 脳波の基礎'
  ...
  description: 測定への入り口として、EEG が何を測定するかを確認できます。
- label: 'Wiki: EEG の前処理と QC'
  url: /wiki/eeg-preprocessing-and-qc.html
  description: 組織化段階で結果を変えるものを補償します。
- label: 'Wiki: 検証インフラストラクチャの基本'
  url: /wiki/verification-basics.html
  description: 最終検証ステップで修正すべき部分を補います。
recommended_pages:
- label: 用語集
  url: /glossary.html
- label: 脳波検査の概要
...
<p>
2 番目の初心者の読みすぎも残っています。より多くのセンサー、同一脳のリンケージ、またはコネクトームの事前分布は、あたかも候補セットがほぼ閉じられているかのように聞こえる可能性があります。このサイトでは、それはまだ強すぎます。同一セッションのマルチモーダル作業には <a href="multimodal-integration-basics.html">Fusion Card</a> が必要で、連続したセイムブレインまたはクロスデイクレームには <a href="state-continuity-bridge.html">State-Continuity Bridge Card</a> が必要で、コネクトーム制約のある予測子には <a href="observation-to-estimation.html#identifiability-card">Identifiability Card</a> が必要です。詳細なルールは <a href="observation-to-estimation.html">Wiki: 観測から推定まで </a> にあります。
</p>
</div>
...
<h2 class="section-title">4 レベルで表示</h2>
...
<h2 class="section-title"> 分割する必要があった 3 つのコンパニオン カード</h2>
<p>
...
<h2 class="section-title">1。観察: まず信号</h2>を取得します。
<p>
...
<h2 class="section-title">2。整理: 信号をそのまま信じないでください</h2>
<p>
...
<h2 class="section-title">3。推定:</h2> の観測からどの程度のことが分かるか
<p>
...
<p>
DCM は候補生成モデルの比較であり、SCM は介入と反事実の説明を容易にする言語です。因果等価クラスは観察データのみから残ることが多いため、候補モデル空間、家族比較、外部検証、介入データの有無を個別に読み取る必要があります。詳細については、<a href="observation-to-estimation.html">Wiki: 観測から推定まで</a> を参照してください。
</p>
</div>
...
<p>
このページは初心者用マップです。潜在交絡監査、ノード定義ポリシー、棄権境界を含む完全な DCM / 有効接続提出ルールが必要な場合は、<a href="observation-to-estimation.html#effective-connectivity-route-card">Wiki: 観察から推定まで</a> に進んでください。
</p>
</div>
</section>

<section class="section" id="verification">
<h2 class="section-title">4。検証: 推定値を信頼する方法</h2>
<p>
...
<h2 class="section-title">このプロセスから学んだこと、そしてまだ不明なこと</h2>
<table class="data-table">
<thead>
<tr>
<th>私たちが知っていること</th>
...
<p>
簡単な定義に戻るには<a href="../glossary.html">用語集</a>を、脳波の役割をもう一度読むには<a href="../eeg_101.html">EEG</a>を、比較検証に進むには<a href="../verification.html">検証インフラストラクチャ</a>を使用してください。
</p>
</section>
