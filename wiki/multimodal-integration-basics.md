...
description: マルチモーダルスタックが何を強化できるのか、何がモデル条件付けのままなのか、そしてなぜ同期取得だけでは 1 つの検証済みの生物学的変数が作成されないのかについて説明します。
article_type: ウィキ
subtitle: 共有クロック、共有ファクター、数量ブリッジは異なる成果です
author: マインドアップロード研究プロジェクト
last_updated: '2026-04-03'
note: 技術/自然科学のみ (フュージョン カードの一貫性のために 2026 年 4 月 3 日に書き直されました)
audience: 多角的な証拠を技術的および自然科学の側面のみから判断したい読者
reading_time: 12～18分
page_intro: このページでは、EEG、MEG、fMRI、PET、fNIRS、侵襲的記録、および MRI を組み合わせた場合に、現在の一次文献が実際に何をサポートしているかを整理します。その目的は、デフォルトで融合を称賛することではなく、同期、ジオメトリ、生理学、モデル負荷を分離し、バンドルの堅牢性を監査可能な部分に分割することです。
accuracy_note: このページは、モダリティを追加すると状態完全な読み出しが可能になると主張するものではありません。どの保険金請求の上限が上がり、どれが上がらないのか、またその理由について説明します。
page_highlights:
- 11 の個別の監査ゲートを通じてマルチモーダル統合を読み取ります。
- 同一セッションでの取得は自己検証型フュージョンとして扱われません。フュージョンカードは引き続き必要です。
- 共有タイムスタンプ、共有因子、および検証済みの生物学的量ブリッジは、別々に保持されます。
- 時間的カーネルの不一致は依然として明らかです。イベントロック電気生理学、血行動態応答ウィンドウ、および分単位の PET ルートは、異なる時間的オブジェクトです。
- 共有された低周波構造は、純粋な迷惑物または純粋な神経標的として自動ラベル付けされません。生理学関連、モダリティ固有、または未解決として分類する必要があります。
- バンドルのゲインはバンドルの堅牢性とは別に保たれます。完全なケースのスライス、モダリティの欠落ポリシー、転送ウィンドウ、およびハードサブグループの不一致は別個の監査です。
- EEG + fMRI および EEG + fNIRS では、血行動態の振幅を神経の差異として読み取る前に、血管状態 / CVR または表面信号の開示が依然として必要です。
- トリモーダル EEG-PET-MRI は、統合されたグラウンド トゥルースではなく、マルチタイムスケールの生理学の読み取りを強化します。
- EEG + 侵襲的記録は、脳全体の真実ではなく、範囲が限定された校正ルートのままです。
- OPM-MEG は、場の制御、校正、およびソースモデルの仮定が開示されている場合にのみ、動きに耐性のあるマクロ電気生理学として解釈されます。
- マルチモーダル融合後でも、シナプス、グリア、転写、および維持状態変数の隠れ状態の完全性は未解決のままです。
known_points:
...
<p>
こちらは<a href="../verification.html#fusion-card">Verification:Fusion Card</a>の背景ページです。検証により動作ルールが得られます。このページでは、なぜルールが厳格でなければならないのか、現在どのルートがより高い測定値を獲得しているのか、そして請求の上限がどこで止まるのかについて説明します。
</p>
</div>
...
<h2 class="section-title">最初に修正するための 11 の監査ゲート</h2>
...
<h2 class="section-title">各主要ルートについて現在の一次文献がサポートしているもの</h2>
<table class="data-table">
<thead>
<tr>
<th>ルート</th>
...
<h2 class="section-title">各ルートが実際に追加するもの</h2>
...
<h2 class="section-title">このサイトで採用されているルールの閲覧</h2>
<div class="key-points">
<h4>ルール</h4>
<ul>
<li><strong>multimodal:</strong> 「複数のモダリティが追加された」ではなく、「どの監査ゲートを通過したか」と読み替えてください。
<li><strong>same-session / atlas-informed:</strong> <a href="../verification.html#fusion-card">Fusion Card</a> が取得関係、ラグ監査、時間カーネル関係、共同登録スコープ、共有対固有ロジック、単峰性 / 事前のみのベースライン、可用性スライス、および外部キャリブレーションを開示しない限り、これを 1 つの検証済みの生物学的状態変数として読み取らないでください。</li>
...
<li><a href="event-sync-and-measurement-logs.html">イベント同期および測定ログ →</a></li>
...
<h4>公開ページ</h4>
<ul>
<li><a href="../eeg_101.html#multimodal-integration">EEG 101: マルチモーダル統合 →</a></li>
<li><a href="../verification.html#fusion-card">検証：フュージョンカード→</a></li>
<li><a href="../tech_roadmap.html#qa-m5">ロードマップ: M5 マルチモーダル統合 → </a></li>
</ul>
</div>
</aside>
</main>
