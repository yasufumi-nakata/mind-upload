---
layout: default
title: 'Wiki: 不確実性、校正、および棄権'
description: 逆ファミリの不確実性オブジェクトと検証ボードを別々に入力する必要がある理由を含め、EEG ソース イメージング、EEG 分類、言語デコード、および閉ループ BCI における不確実性、キャリブレーション、および棄権について整理します。
article_type: ウィキ
subtitle: 自信は信頼性と同じではありません。キャリブレーションとカバレッジは個別に読む必要があります。
author: マインドアップロード研究プロジェクト
last_updated: '2026-03-31'
note: 技術/自然科学のみ
audience: ポイント推定のみの主張を避けたい人、および実用的な指標としての調整と棄権を理解したい人。
reading_time: 12～18分
page_intro: 'このページでは、一次文献を使用して、EEG ソース イメージング、EEG 分類、言語デコード、および閉ループ BCI における不確実性、信頼区間、キャリブレーション、および棄権を整理します。それは、数値が表示されたときにその数値をどの程度信頼するかということだけでなく、どの出力空間が事前に固定されているか、どの不確実性オブジェクトが実際に報告されているか、いつ出力を停止すべきかについても重要です。

  '
accuracy_note: 'このページでは、統計における教科書の完全性よりも、技術的および自然科学の監査項目を優先します。一部の厳密な定義は圧縮されていますが、核心点は明確なままです。つまり、信頼性とキャリブレーションは異なり、カバレッジを下げることでリスクを下げることができ、再キャリブレーションの負担自体がオンライン
  システムのパフォーマンス指標です。

  '
page_highlights:
- 信頼性とキャリブレーションは異なり、ソフトマックスや事後スコアなどの単一の数値から安全性を判断することはできません。
- 同じ ECE とカバレッジは、セッション内、セッション間、被験者間、および時間的シフト設定内で異なる意味を持ちます。
- 脳からテキストまたは音声へのデコードでは、信頼性は、宣言された候補バンク、開始レジーム、即時足場、および事前の言語によって条件付けされます。
- キューサポートされたセマンティック BCI、視聴/リコールされたコンテンツのキャプション、既知のオンセット単語のデコード、プロンプト条件付き生成、およびストリーミング音声合成は、1 つの信頼度オブジェクトを共有しません。
- 開示すべき不確実性指標は、ソース画像化、脳波分類、言語解読、閉ループ BCI ごとに異なります。
- 内部ソースイメージング、クロスパイプラインスプレッド、フォワードモデル感度、焦点事後サポート、まばらな偏りのない間隔、および範囲を意識した不確実性は、交換可能な 1 つの幅ではありません。
- 名前付き外部バリデータは、ソース イメージングにおける不確実性オブジェクトの一部です。刺激部位、同時 SEEG、および他のボードは、同じエラー質問に答えません。
- 棄権は弱さではありません。これは、補償範囲とリスクの間の交換条件を明らかにする操作です。
known_points:
...
<h2 class="section-title">最初に修正される 4 つの監査ゲート</h2>
...
<h2 class="section-title">信頼性、間隔、校正、棄権を混同しないでください</h2>
<table class="data-table">
<thead>
<tr>
<th>コンセプト</th>
...
<h2 class="section-title">キャリブレーションはフィット/キャリブレーション/テスト用に個別に管理されます</h2>
<table class="data-table">
<thead>
<tr>
<th>ステージ</th>
...
<h2 class="section-title">同じキャリブレーションでも評価ファミリが異なれば意味も異なります</h2>
<table class="data-table">
<thead>
<tr>
<th>評価ファミリー</th>
...
<h2 class="section-title">確率、間隔、予測セット、および棄権は別個の出力</h2>
<table class="data-table">
<thead>
<tr>
<th>出力タイプ</th>
...
<h2 class="section-title">候補銀行と即時条件付きの信頼はオープンワールドの不確実性ではない</h2>
<table class="data-table">
<thead>
<tr>
<th>ルートタイプ</th>
...
<h2 class="section-title">銘柄ごとに公開する指標を変更</h2>
<table class="data-table">
<thead>
<tr>
<th>Tタスク</th>
...
<li><strong>キャリブレーションとアンプの取り付け確率、間隔、予測セット、および棄権を強調表示する結果への棄権カード:</strong> <a href="../verification.html#calibration-abstention-card">Verification</a> 側の共通送信のスプリット、スライス、カバレッジ リスク、およびフォールバック ポリシーを修正しました。 </li>
<li><strong>出力が言語に似ている場合は、ニューラル貢献カードをスタックします。</strong>テキスト/音声/脳からテキストへの出力の場合、<a href="../verification.html#neural-contribution-card">Verification</a> 側での候補セット、プロンプト、および脳なし / LM なし / シャッフル開示と一緒にのみスコアを調整します。 </li>
</ul>
</div>
</section>
...
<p>
ソースイメージング側に戻るには<a href="observation-to-estimation.html">、観測から推定まで</a>をご利用ください。閉ループ側に戻すには、<a href="closed-loop-latency-jitter-and-safety-stops.html">閉ループ、遅延、ジッター、および安全停止</a>を使用してください。公開ルール全体に戻るには、<a href="../verification.html">Verification platform</a> を使用してください。
</p>
</section>
...
<h4>公開ページ</h4>
<ul>
<li><a href="../eeg_101.html">EEG の基本 -></a></li>
