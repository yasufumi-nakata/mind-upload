...
description: 神経科学と WBE の見出しを主張の強さ、証拠のクラス、および必要なルート カードに変換するための初心者向けガイド。
article_type: ウィキ
subtitle: 強い言葉は安いです。強力な証拠はありません。
author: マインドアップロード研究プロジェクト
last_updated: '2026-04-01'
note: 読書ガイド
audience: 神経科学や WBE の主張を深読みしたくない人、および L0 ～ L5 へのより厳密な初心者ルートを望む読者
reading_time: 15～20分
page_intro: このページでは、派手な神経科学と WBE の文言を、主張の強さ、証拠のクラス、および主張が強く読み取れるようになる前に必要な最小限のルート カードに戻します。目標は進歩を阻害することではなく、スコアのインフレと証拠クラスの崩壊を入り口で阻止することです。
accuracy_note: このページはテクノロジーと自然科学の側面に留まります。ヘッドライン、ハイスコア、または 1 つのモデリング フレームワークを、開示された測定条件、外部検証、または棄権の代替として扱うものではありません。
page_highlights:
- L0 ～ L5 は、興奮レベルではなく、主張の強さとして読み取る必要があります。
- サブジェクト/セッションのフィンガープリント、取得と配布のシフト、または言語/タスクの事前情報が未解決のままである場合、高いデコード精度はターゲット固有の証拠ではありません。
- プロキシが豊富な人間のマルチモーダルな証拠は、デフォルトでは同一被験者状態の閉鎖ではありません。行ごとに直接観察可能な構造、共有構造と固有構造、共通ドライバー監査、プロキシ クラス、運用の成熟度、およびキャリブレータの役割を開示する必要があります。
- 生体トラクトグラフィーの結果は、トラクトグラフィールートカードが開示されていない限り、事前のマクロパスウェイです。
- 有効接続性グラフは、候補モデル空間、観察されたサブシステムの閉包/潜在交絡の監査、ノード定義ポリシー、サンプリング/変換の感度、検証、信頼性、および棄権が開示されない限り、モデル条件付けされたままになります。
- 不可逆性論文はすべて同じ熱力学量を測定するわけではありません。 estimator ファミリと null コントロールが重要です。
- リアルタイムの閉ループには依然として身体/環境の境界カードが必要です。低遅延だけでは十分ではありません。
known_points:
...
- label: 'Wiki: WBE の基本'
  ...
  description: 請求項を読む前に全体的な技術的な全体像を設定します。
- label: 'Wiki: 検証インフラストラクチャの基本'
  url: /wiki/verification-basics.html
  description: ルート カード、監査、事前登録が運用上重要である理由を説明します。
- label: 'Wiki: 観察から推定へ'
  url: /wiki/observation-to-estimation.html
  description: ESI、DCM、SCM がすべて同じ種類の主張をサポートしていない理由を説明します。
- label: 'Wiki: 測定スタックの可観測性とクレームの上限'
  url: /wiki/measurement-stack-and-claim-ceiling.html
  description: 各モダリティが直接観察する内容と、請求の上限がどこに現れるかを示します。
recommended_pages:
- label: WBE の概要
  url: /wbe_101.html
- label: 検証プラットフォーム
...
<h2 class="section-title">技術的なレベルを失うことなく、L0 ～ L5 を日常言語で言い換える</h2>
...
<h2 class="section-title">一般的な見出しフレーズをより安全な最初の読みに変換する</h2>
<table class="data-table">
<thead>
<tr>
<th>見出しスタイルのフレーズ</th>
...
<h2 class="section-title">なぜ初心者ルールを厳しくしなければならなかったのか</h2>
...
<h2 class="section-title">見出しの強力なバージョンを信じる前の 7 つの質問</h2>
...
<h2 class="section-title">この初心者ページの次に進むべき場所</h2>
<table class="data-table">
<thead>
<tr>
<th>見出しの主な内容が...</th>
...
<td><a href="observation-to-estimation.html#effective-connectivity-route-card">有効接続ルートカード</a></td>
<td>候補とモデルの依存関係が始まりにすぎない理由、およびクロージャ、ノード定義、サンプリング、検証が依然として開示されなければならない理由を示しています。</td>
</tr>
<tr>
<td>エントロピー生成 / 不可逆性 / 時間の矢</td>
<td><a href="thermodynamic-grounding-basics.html#irreversibility-route-card">不可逆性ルートカード</a></td>
<td>エスティメーター ファミリとヌル コントロールを分離します。</td>
</tr>
<tr>
<td>クローズドループ / BCI / 実施例</td>
<td><a href="closed-loop-latency-jitter-and-safety-stops.html">閉ループ、レイテンシー、ジッター、安全停止</a></td>
