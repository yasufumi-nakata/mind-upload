...
description: "dataset version、BIDS、Validator、QCログ、分割ルール、ベースライン、実行手順、失敗例など、L0 の最低成果物を整理します。"
article_type: Wiki
...
page_highlights:
  - "dataset version、BIDS、Validator、QC、分割、ベースライン、実行手順、失敗例を 1 パックとして整理します。"
...
<h2>まず 1 パックで考える</h2>
<p>
L0 の成果物は、1 つのファイルや 1 つのスコアではありません。<strong>どの版の入力か</strong>、<strong>入力の形</strong>、<strong>品質の証跡</strong>、<strong>評価のルール</strong>、<strong>再実行手順</strong>がそろって、初めて第三者が追える状態になります。
</p>
</div>

<section class="section" id="artifact-pack">
<h2 class="section-title">L0 で最低限そろえたい 8 点</h2>
...
<tbody>
<tr>
<td><strong>1. データ版</strong></td>
<td>OpenNeuro snapshot、PhysioNet version、DOI、取得日、または永続 URL。</td>
<td>同じ dataset 名でも別版を混ぜ、再現不能になります。</td>
</tr>
<tr>
<td><strong>2. データ骨格</strong></td>
<td>BIDS の骨格、最低限のメタデータ、課題名、電極情報。</td>
<td>同じ入力を別の人が持てず、比較の出発点が揃いません。</td>
</tr>
<tr>
<td><strong>3. 規格確認結果</strong></td>
<td>BIDS Validator の結果、残した警告の理由。</td>
<td>共有不能な規格違反を見落としやすくなります。</td>
</tr>
<tr>
<td><strong>4. QC ログ</strong></td>
<td>欠損、ノイズ、アーティファクト、除外理由の数値記録。</td>
<td>どのデータが悪かったかが人依存になり、再現性が壊れます。</td>
</tr>
<tr>
<td><strong>5. 分割ルール</strong></td>
<td>within-session / cross-session / cross-subject の別、train/test の単位、リーク防止ルール、test を触らない約束。</td>
<td>数字の上振れを見抜けず、評価全体が崩れます。</td>
</tr>
<tr>
<td><strong>6. ベースライン結果</strong></td>
<td>単純な 1 本の指標、設定、結果表。</td>
<td>改善を主張する出発点が無くなります。</td>
</tr>
<tr>
<td><strong>7. 実行手順</strong></td>
<td>コマンド、環境、乱数、前処理条件、既知のつまずきどころ。</td>
<td>他の人が同じ流れを再実行できません。</td>
</tr>
<tr>
<td><strong>8. 失敗例</strong></td>
...
<h4>Pack</h4>
<ul>
<li><strong>Input:</strong> 版固定 + BIDS の形</li>
...
<tbody>
<tr>
<td>他の人が同じ版の入力を持てますか</td>
<td>snapshot / version / DOI と BIDS 骨格が揃っています。</td>
<td>データ版と骨格を先に固定します。</td>
</tr>
<tr>
