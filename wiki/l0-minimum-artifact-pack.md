---
layout: default
title: 'Wiki: L0 の最小アーティファクト パック'
description: データセット ID、EEG-BIDS スケルトン、イベント忠実度、ベンチマーク オブジェクトとメトリック バンドル、ベンチマーク ガバナンス スナップショット、一時的有効性の付録、取得と配布の監査、派生リネージュ、ワークフロー
  レシピ、ベースライン、再生ステップなどの最小限の L0 成果物を整理します。
article_type: ウィキ
subtitle: バージョン、可観測性、ベンチマークの意味、リネージ、およびリプレイが修正されるまでは、再現可能とは呼ばないでください。
author: マインドアップロード研究プロジェクト
last_updated: '2026-04-03'
note: 操作ガイド / 時間的有効性の同期
audience: L0の作成を始めた人や、どこまで再現性のある解析と言えるのか確認したい人。
reading_time: 12～18分
page_intro: このページは、このサイトで L0 結果が再現可能な分析と呼ばれる前に、何をバンドルする必要があるかを修正する補助ページです。これは手順書ではありません。これは、サードパーティがスコアだけでなく、実際に観察された内容、どの予測オブジェクトとメトリクス
  バンドルが使用されたか、どのベンチマーク ルールが適用されていたか、何が適用され、何が範囲外に残ったかを再構築できるかどうかを問う提出形式のチェックです。
accuracy_note: このページでは、L0 の現在の最小値を定義します。それ自体は因果関係や同一性の主張を正当化するものではありませんが、これらのフィールドがなければ、L0 の比較可能性さえも弱すぎるままです。
page_highlights:
- L0 パックは、単なるバージョン + BIDS + QC + スプリット + ベースラインではなくなりました。これには、ベンチマーク オブジェクト + メトリクス バンドル、ベンチマークの出所/ガバナンス、ワークフロー レシピ、およびクレームが複数のセッション、日、または適応ステージにまたがる場合の一時的有効性に関する付録も含まれるようになりました。
- このパックは依然として 5 つのバンドルを中心に構成されていますが、評価バンドルでは、スプリット ファミリ、予測オブジェクト、メトリック セマンティクス、現在のベンチマーク ルール、時間的範囲、セットアップ分布、およびベースラインが明示的に分離されています。
- 予測オブジェクト、独立した予測ユニット、グループ化されたホールドアウト ユニット、および運用予算がガバナンスと並行して凍結されない限り、ベンチマーク名は依然として粗すぎます。
- 現在のルールのスナップショットとその後の主催者の修正がスコアとともに凍結されない限り、チャレンジまたはリーダーボードの結果はこのサイトで再現可能な成果物ではありません。
- 派生系統は依然としてワークフローの出自と同じではありません。`GeneratedBy` / `SourceDatasets`、設定ファイル、およびランタイム ピンを一緒に凍結する必要があります。
- クロスセッションまたはアダプテーションの命名はまだ一時的に有効ではありません。状態アノテーションは、高速ラベルと低速内部環境の開示を分割するようになりましたが、固定デコーダ間隔、再調整負荷、および転送上限は引き続き開示する必要があります。
- このページは、データセットと検証ですでに使用されているより厳格な実践的なルールと同期されるようになりました。
known_points:
...
<h2 class="section-title">L0 パックには最低 14 個のアイテムが必要になります</h2>
...
<h2 class="section-title">なぜ古い8ポイントパックは弱すぎるのか</h2>
<table class="data-table">
<thead>
<tr>
<th>ウィークポイント</th>
...
<h2 class="section-title">まとめて保管できる 5 つのバンドル</h2>
...
<h2 class="section-title">一般的な省略</h2>
<table class="data-table">
<thead>
<tr>
<th>一般条件</th>
...
<h2 class="section-title">A より厳格な L0 完了チェック</h2>
<table class="data-table">
<thead>
<tr>
<th>質問</th>
...
<p>
実際の手順に従いたい場合は <a href="../datasets.html#l0-practice">Hands-On</a> に戻り、入力データを再選択したい場合は <a href="../datasets.html">Data & Bench</a> に戻り、この製品が公共財としてどのように評価されるかを確認したい場合は <a href="../verification.html">Verification Infrastructure</a> に戻ってください。
</p>
</section>
