---
layout: default
title: 'Wiki: データの分割とデータ漏洩'
description: トレーニング/テストの分割、サブジェクト/セッション/時間のリーク、取得と配布のショートカット、ベンチマークとガバナンスの障害モードを最初から説明します。
article_type: ウィキ
subtitle: 精度が高くても分類が甘ければ評価は崩れる
author: マインドアップロード研究プロジェクト
last_updated: '2026-03-25'
note: 実践ガイド
audience: 公開データを利用して初期評価を作成する人、漏洩や分裂に混乱しやすい人
reading_time: 12～18分
page_intro: このページはデータセットの分割方法とデータ漏洩がなぜ危険なのかを一から解説したWikiです。 2026-03 年の再監査では、もう 1 つの点が強化されました。きれいな分割は必要ですが、買収と分配の近道やベンチマークとガバナンスの失敗が隠されたままであれば、それだけではまだ十分ではありません。
accuracy_note: これらは運用ルールであり、万能の公式ではありません。最適な分割は依然としてタスクとデータ構造に依存しており、公式チャレンジの操作によってベンチマーク スコアの意味が大きく変わる可能性があります。
page_highlights:
- 単元を科目別、回別、時間別に分けるかによって難易度は大きく異なります。
- リークは「不正行為」だけでなく、善意による前処理や分割によっても発生します。
- 4 つのスターター データセット間でも、主題、症例、夜間、セッションなど、独立した単位は異なります。
- まず最初に注目すべきは、精度そのものよりも分割ルールとリーク対策です。
- クリーンな被験者/セッション分割でも、サイト、デバイス、参照システム、電極レイアウト、またはプロトコルの組み合わせなどの取得と分配のショートカットは無効になりません。
- ベンチマーク ガバナンスは漏洩制御の一部です。バージョン、ランダム化、非表示グループ化、追加データ ポリシー、事前トレーニングされたチェックポイント、推論段階の制限、およびオーガナイザーの事後分析はすべて、スコアの意味を変更する可能性があります。
known_points:
...
- label: 'Wiki: EEG の前処理と QC'
  ...
  description: 前処理自体が結果を変更する部分を補正します。
- label: 'Wiki: EEG 基礎モデルと事前トレーニング'
  url: /wiki/eeg-foundation-models.html
  description: ベンチマーク ルール、追加の事前トレーニング、リーダーボードの解釈が曖昧になり始めた場合は、このページを使用してください。
- label: 'Wiki: 検証インフラストラクチャの基本'
  url: /wiki/verification-basics.html
  description: 漏洩防止が「業務の一部」である理由をご覧ください。
- label: ウィキホーム
  url: /wiki/
  description: 学習ページの全体マップに戻ることができます。
recommended_pages:
- label: データとベンチ
  url: /datasets.html
- label: 実践
...
<h2 class="section-title">パーティション分割がそれほど重要な理由</h2>
<p>
...
<h2 class="section-title">まずは割り算の単位を意識する</h2>
...
<h2 class="section-title">4 スターター データ項目、独立したユニットは同じではありません</h2>
<div class="note-box">
...
<h2 class="section-title">7 一般的なリークパターン</h2>
<table class="data-table">
<thead>
<tr>
<th>よくある事故</th>
...
<h2 class="section-title">分割衛生でも 4 つのショートカット ファミリーが残る</h2>
<table class="data-table">
<thead>
<tr>
<th>ショートカットファミリー</th>
...
<h2 class="section-title">今回追加されたデータセット固有のリーク警告</h2>
<table class="data-table">
<thead>
<tr>
<th>データセット</th>
...
<h2 class="section-title">せめてこれだけは報告したい</h2>
...
<h2 class="section-title">1冊目で迷った時の安全対策</h2>
<p>
...
<p>
実際のスターター データを確認したい場合は <a href="../datasets.html">Data & Bench</a> に戻り、最小限のループの作成に戻りたい場合は <a href="../datasets.html#l0-practice">Hands-on</a> に戻り、これが検証基盤の一部である理由を確認したい場合は <a href="../verification.html">Verification Foundation</a> に戻ってください。
</p>
</section>
...
<li><a href="eeg-foundation-models.html">EEG 基礎モデルと事前トレーニング →</a></li>
