...
## 2026-03-14 11:02 JST 再検証ログ（本run）

- `research_harvest_50.md` 由来の `Ux-RQy` 行は `60件`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を再確認。
- 本ページ内で参照している EEG-DATA は `24 ID`（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）で、`EEG-DATA/eeg_dataset_summary_ja.csv` 上の欠落 `0` を確認。
- 助成キー `G1-G6` は参照系統を分離して一致確認（`G1-G2=grant_queue/*.yaml`、`G3-G6=auto-research-funds/wiki/cards + grant_eeg_dataset_match.csv`）。
- 運用方針は継続して `1RQ=1検証命題=1応募テーマ`。`A/B` は実験計画、`C` は制度・監査設計で出す。
- 前版ログにあった EEG参照ID件数の記載ゆれ（`17`）は、表本体に合わせて `24` に補正済み。

## 2026-03-14 12:03 JST 再検証ログ（本run）

- `mind-upload-eeg-rq60-feasibility-and-funds.md` の RQ行数は `60件` を再確認（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）。
...
## 2026-03-14 16:02 JST 再検証ログ（本run）

- `research_harvest_50.md` と本ページの `RQ` を `U` 別で再照合し、`60件`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）の一致を確認。
- `A/B/C` 判定は `17/25/18` を再計数し、前runから差分なし。
- 参照 `EEG-DATA 24 ID`（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は `EEG-DATA/eeg_dataset_summary_ja.csv` で欠落 `0` を確認。
- 助成ID `G1-G6` と追加候補ID（`1geD73ZENwB8yaor`, `lEYmRP97ng3perjO`, `1geD73Z9K2B8yaor`, `QKYgNPxZxN38bO1Z`, `46z9VPE6E0BrvEJR`, `1geD73ZE54B8yaor`, `1geD73Z6yq38yaor`）は `auto-research-funds` 内で実在ヒットを再確認。
- 本runでも方針は `1RQ=1検証命題=1応募テーマ` を維持し、汎用要約ではなく各行を直接実験手順へ落とせる粒度で運用する。

## 2026-03-14 17:01 JST 再検証ログ（本run）

- `mind-upload/automation/rq_reading_path_timed_round19_2026-03-01.csv` を `Ux-RQy` 単位で再読し、本ページの60行が元RQ文と1対1対応していることを再確認。
- `各RQの3点セット（A/B/C判定, 応募テーマ, 推奨EEG-DATA ID）` を全行で点検し、欠落列 `0`・空欄 `0` を確認。
- `A/B/C=17/25/18`、`参照EEG-DATA=24 ID`（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は前runから不変。
- 助成先は `auto-research-funds` 側の `G1-G6 + 追加候補ID` を維持し、`A/B` は技術実証トラック、`C` は制度・法務連携トラックとして使い分ける方針を固定。

## 2026-03-14 18:10 JST 追記（RQ別・初手実験カード）

全60RQを『1問ずつ実行可能』にするため、既存の判定/テーマ/データを崩さず、各RQに初手実験手順と失敗時アクションを追加しました。

| RQ | 主要指標（2つ） | 最初の実験手順（主データ） | 失敗時の次アクション |
|---|---|---|---|
| U0-1 | `介入一致率` / `反事実一致率` | `11` を主データに固定し、`介入一致率` と `反事実一致率` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U0-2 | `介入一致率` / `反事実一致率` | `11` を主データに固定し、`介入一致率` と `反事実一致率` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U0-3 | `介入一致率` / `反事実一致率` | `11` を主データに固定し、`介入一致率` と `反事実一致率` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U0-4 | `介入一致率` / `反事実一致率` | `11` を主データに固定し、`介入一致率` と `反事実一致率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U1-1 | `位置誤差(mm)` / `事後分布幅` | `56` を主データに固定し、`位置誤差(mm)` と `事後分布幅` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U1-2 | `位置誤差(mm)` / `事後分布幅` | `56` を主データに固定し、`位置誤差(mm)` と `事後分布幅` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U1-3 | `位置誤差(mm)` / `事後分布幅` | `56` を主データに固定し、`位置誤差(mm)` と `事後分布幅` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U1-4 | `位置誤差(mm)` / `事後分布幅` | `56` を主データに固定し、`位置誤差(mm)` と `事後分布幅` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U3-1 | `予測改善率(ΔR2)` / `外挿性能` | `13` を主データに固定し、`予測改善率(ΔR2)` と `外挿性能` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U3-2 | `予測改善率(ΔR2)` / `外挿性能` | `13` を主データに固定し、`予測改善率(ΔR2)` と `外挿性能` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U3-3 | `予測改善率(ΔR2)` / `外挿性能` | `13` を主データに固定し、`予測改善率(ΔR2)` と `外挿性能` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U3-4 | `予測改善率(ΔR2)` / `外挿性能` | `13` を主データに固定し、`予測改善率(ΔR2)` と `外挿性能` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U3-5 | `予測改善率(ΔR2)` / `外挿性能` | `13` を主データに固定し、`予測改善率(ΔR2)` と `外挿性能` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U3-6 | `予測改善率(ΔR2)` / `外挿性能` | `13` を主データに固定し、`予測改善率(ΔR2)` と `外挿性能` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U4-1 | `介入効果一致率` / `因果辺再現率` | `19` を主データに固定し、`介入効果一致率` と `因果辺再現率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U4-2 | `介入効果一致率` / `因果辺再現率` | `19` を主データに固定し、`介入効果一致率` と `因果辺再現率` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U4-3 | `介入効果一致率` / `因果辺再現率` | `19` を主データに固定し、`介入効果一致率` と `因果辺再現率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U4-4 | `介入効果一致率` / `因果辺再現率` | `19` を主データに固定し、`介入効果一致率` と `因果辺再現率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U7-1 | `同期誤差(ms)` / `再現率低下(pt)` | `676` を主データに固定し、`同期誤差(ms)` と `再現率低下(pt)` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U7-2 | `同期誤差(ms)` / `再現率低下(pt)` | `676` を主データに固定し、`同期誤差(ms)` と `再現率低下(pt)` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U7-3 | `同期誤差(ms)` / `再現率低下(pt)` | `676` を主データに固定し、`同期誤差(ms)` と `再現率低下(pt)` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U7-4 | `同期誤差(ms)` / `再現率低下(pt)` | `676` を主データに固定し、`同期誤差(ms)` と `再現率低下(pt)` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U7-5 | `同期誤差(ms)` / `再現率低下(pt)` | `676` を主データに固定し、`同期誤差(ms)` と `再現率低下(pt)` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U7-6 | `同期誤差(ms)` / `再現率低下(pt)` | `676` を主データに固定し、`同期誤差(ms)` と `再現率低下(pt)` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U8-1 | `安定率` / `回復時間(s)` | `6` を主データに固定し、`安定率` と `回復時間(s)` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U8-2 | `安定率` / `回復時間(s)` | `6` を主データに固定し、`安定率` と `回復時間(s)` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U8-3 | `安定率` / `回復時間(s)` | `6` を主データに固定し、`安定率` と `回復時間(s)` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U8-4 | `安定率` / `回復時間(s)` | `6` を主データに固定し、`安定率` と `回復時間(s)` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U8-5 | `安定率` / `回復時間(s)` | `6` を主データに固定し、`安定率` と `回復時間(s)` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U8-6 | `安定率` / `回復時間(s)` | `6` を主データに固定し、`安定率` と `回復時間(s)` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U10-1 | `理論整合スコア` / `proxy相関(ρ)` | `735` を主データに固定し、`理論整合スコア` と `proxy相関(ρ)` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U10-2 | `理論整合スコア` / `proxy相関(ρ)` | `735` を主データに固定し、`理論整合スコア` と `proxy相関(ρ)` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U10-3 | `理論整合スコア` / `proxy相関(ρ)` | `735` を主データに固定し、`理論整合スコア` と `proxy相関(ρ)` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U10-4 | `理論整合スコア` / `proxy相関(ρ)` | `735` を主データに固定し、`理論整合スコア` と `proxy相関(ρ)` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U11-1 | `理論間順位一致率` / `偽陽性率` | `735` を主データに固定し、`理論間順位一致率` と `偽陽性率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U11-2 | `理論間順位一致率` / `偽陽性率` | `735` を主データに固定し、`理論間順位一致率` と `偽陽性率` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U11-3 | `理論間順位一致率` / `偽陽性率` | `735` を主データに固定し、`理論間順位一致率` と `偽陽性率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U11-4 | `理論間順位一致率` / `偽陽性率` | `735` を主データに固定し、`理論間順位一致率` と `偽陽性率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U12-1 | `監査ログ完全性` / `同意撤回反映時間` | `11` を主データに固定し、`監査ログ完全性` と `同意撤回反映時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U12-2 | `監査ログ完全性` / `同意撤回反映時間` | `11` を主データに固定し、`監査ログ完全性` と `同意撤回反映時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U12-3 | `監査ログ完全性` / `同意撤回反映時間` | `11` を主データに固定し、`監査ログ完全性` と `同意撤回反映時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U12-4 | `監査ログ完全性` / `同意撤回反映時間` | `11` を主データに固定し、`監査ログ完全性` と `同意撤回反映時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U12-5 | `監査ログ完全性` / `同意撤回反映時間` | `11` を主データに固定し、`監査ログ完全性` と `同意撤回反映時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U12-6 | `監査ログ完全性` / `同意撤回反映時間` | `11` を主データに固定し、`監査ログ完全性` と `同意撤回反映時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U13-1 | `意味一致率` / `機構識別率` | `509` を主データに固定し、`意味一致率` と `機構識別率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U13-2 | `意味一致率` / `機構識別率` | `509` を主データに固定し、`意味一致率` と `機構識別率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U13-3 | `意味一致率` / `機構識別率` | `509` を主データに固定し、`意味一致率` と `機構識別率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U13-4 | `意味一致率` / `機構識別率` | `509` を主データに固定し、`意味一致率` と `機構識別率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U13-5 | `意味一致率` / `機構識別率` | `13` を主データに固定し、`意味一致率` と `機構識別率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U13-6 | `意味一致率` / `機構識別率` | `509` を主データに固定し、`意味一致率` と `機構識別率` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U14-1 | `追試成功率` / `再現コスト増分` | `6` を主データに固定し、`追試成功率` と `再現コスト増分` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U14-2 | `追試成功率` / `再現コスト増分` | `6` を主データに固定し、`追試成功率` と `再現コスト増分` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U14-3 | `追試成功率` / `再現コスト増分` | `6` を主データに固定し、`追試成功率` と `再現コスト増分` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U14-4 | `追試成功率` / `再現コスト増分` | `6` を主データに固定し、`追試成功率` と `再現コスト増分` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U14-5 | `追試成功率` / `再現コスト増分` | `6` を主データに固定し、`追試成功率` と `再現コスト増分` のベースラインを先に計測する。 | 外部要件（追加モダリティ/制度要件）を明示し、段階提案（技術実証→統合検証）へ切り替える。 |
| U14-6 | `追試成功率` / `再現コスト増分` | `6` を主データに固定し、`追試成功率` と `再現コスト増分` のベースラインを先に計測する。 | 同じ主データで前処理・分割固定の再実行を行い、再現率の差分を監査ログへ残す。 |
| U15-1 | `規制要件充足率` / `停止基準発動時間` | `49` を主データに固定し、`規制要件充足率` と `停止基準発動時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U15-2 | `規制要件充足率` / `停止基準発動時間` | `49` を主データに固定し、`規制要件充足率` と `停止基準発動時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U15-3 | `規制要件充足率` / `停止基準発動時間` | `49` を主データに固定し、`規制要件充足率` と `停止基準発動時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
| U15-4 | `規制要件充足率` / `停止基準発動時間` | `49` を主データに固定し、`規制要件充足率` と `停止基準発動時間` のベースラインを先に計測する。 | 性能主張は行わず、制度・監査設計トラックとして提出し、EEGは補助ログ用途に限定する。 |
...
## 2026-03-15 03:03 JST 再検証ログ（本run / pull後の最新版突合）

- `mind-upload` を `f24ca95` まで更新した状態で、`research_harvest_50.md` の `60RQ` を `Ux-RQy` 単位で再照合し、表の件数・順序・問い文の対応を再確認（欠落 `0`、重複 `0`）。
- `EEG-DATA` を `b3fb0aabe3` まで更新し、データセット選定は「RQごとに主データ1本を固定し、補助データを追加」する方針を維持（汎用的な総当たり探索はしない）。
- `auto-research-funds` を `6571303ca` まで更新し、助成テーマは `A/B/C` 判定に応じて `技術実証` と `制度・監査設計` を明確分離して割り当てる運用を再確認。
- 今回も `1RQ=1検証命題=1応募テーマ=1主データ` を固定ルールとして維持し、「各課題を1件ずつ深く理解して記述する」方針で継続する。

## 2026-03-15 04:02 JST 再検証ログ（本run / 最新pull後の実体照合）

- `main` の最新状態で再実行し、`mind-upload=4607efe7fd`、`EEG-DATA=b3fb0aabe3`、`auto-research-funds=6571303caf` を基準コミットとして固定しました。
- `research_harvest_50` 正本と本ページの `Ux-RQy` を再突合し、`RQ行数=60`、`重複=0`、`欠落=0`、`U別内訳=U0(4)/U1(4)/U3(6)/U4(4)/U7(6)/U8(6)/U10(4)/U11(4)/U12(6)/U13(6)/U14(6)/U15(4)` を確認しました。
- `A/B/C` 判定は `17/25/18` で据え置き。`A/B` は技術実証として応募、`C` は制度・監査設計トラックで出す方針を維持しました。
...
- `mind-upload` / `EEG-DATA` / `auto-research-funds` の `main` を pull 後に再点検し、`RQ_TOTAL=60` と `A/B/C=17/25/18` の整合を確認。
...
## 2026-03-16 03:04 JST 再検証ログ（本run / 60RQ判定・助成接続の再固定）

- `research_harvest_50.md` 正本に対して本表（`## U0 リサーチクエスチョン` から `## 実行優先順` 直前）を再カウントし、`RQ_TOTAL=60`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を確認しました。
- 判定内訳を再計数し、`A/B/C=17/25/18` を維持していることを確認しました。
- RQ行で参照する `推奨EEG-DATA` は `24 ID`（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）で、`EEG-DATA/eeg_dataset_summary_ja.csv` 上の欠落 `0` を確認しました。
- 助成実ID `G1-G6` は分離参照で再確認しました（`G1-G2=grant_queue/*.yaml`、`G3-G6=auto-research-funds/wiki/Mind-Upload-EEG-RQ-Grant-Map.md`）。
- 方針を継続します。汎用俯瞰へ戻さず、`1RQ=1検証命題=1応募テーマ=1主データ` の深掘り運用を維持します。

## 2026-03-16 06:03 JST 再検証ログ（本run / pull後の全体整合確認）

- 作業前に `mind-upload=364217a`、`EEG-DATA=99f6801432`、`auto-research-funds=5b3c35e83` まで `main` を `pull --ff-only` で更新しました。
- `Ux-RQy` 行の再カウントで、RQ表は `120行`（判定表60 + 提出固定表60）を維持しました。
- 判定の正本（`mind-upload-rq60-deep-focus-notes.md`）を再照合し、`A/B/C=17/25/18`、`RQ_TOTAL=60` を再確認しました。
- 参照ID `6,11,13,16,19,29,39,49,56,65,509,676,696,719,735,783,842,859,1011,1839,1972,2412,3419,4878` は `EEG-DATA/eeg_dataset_summary_ja.csv` で全件実在（欠落 `0`）を再確認しました。
- 助成キー `G1-G6` の実ID（`GR-2026-013`, `GR-2026-014`, `9Lx4dPK6a4k2gOb7`, `Drbm6vBRDJkn0NGJ`, `871pw3rLjNPKgqA0`, `46z9VPE4wnkrvEJR`）は `grant_queue/*.yaml` と `auto-research-funds/wiki/Mind-Upload-EEG-RQ-Grant-Map.md` の両系統で再確認しました。

### 次に深掘りする6RQ（汎用俯瞰ではなく1件ずつ進める実行順）

1. `U4-2`: 最小介入因果主張の再現可否を固定（主データ `56`）
2. `U7-5`: 前処理CIの release block 閾値を固定（主データ `676`）
3. `U8-4`: 異常検知遅延とフェイルセーフ発動率を同時評価（主データ `2412`）
4. `U11-1`: 意識指標3系統の順位保存率を先に固定（主データ `859`）
...
## 2026-03-16 12:02 JST 再検証ログ（本run / pull後・最新データ反映）
...
- 正本 `research_harvest_50.md` と本ページの RQセグメント（`## U0` 〜 `## 実行優先順`）を再照合し、`RQ_TOTAL=60` を再確認しました。
...
- 参照EEGデータID `24件`（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は `EEG-DATA/eeg_dataset_summary_ja.csv` で全件実在（欠落 `0`）でした。
- 実務方針を継続します。汎用俯瞰ではなく `1RQ=1検証命題=1応募テーマ=1主データ` を固定し、各課題を個別に深掘りして申請文へ直接転記します。

## 2026-03-16 13:04 JST 再検証ログ（本run / pull後・RQ単位深掘りの継続固定）
...
- 正本 `research_harvest_50.md` の `リサーチクエスチョン分解` を再計数し、`RQ_TOTAL=60`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を再確認しました。
- 本ページの判定表を再計数し、`A/B/C=17/25/18` を再確認しました。
...
## 2026-03-16 19:04 JST 再検証ログ（本run / pull後・RQ単位深掘り維持）
...
- `mind-upload/research_harvest_50.md` の `リサーチクエスチョン分解` を再カウントし、`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`、合計 `60` を再確認しました。
- 本ページの `RQ` 行は `60` 件、判定内訳は `A/B/C=17/25/18` を維持していることを再確認しました。
...
- 助成キー `G1-G6` は `G1-G2=grant_queue/*.yaml` と `G3-G6=auto-research-funds/grant_eeg_dataset_match.csv` の系統で全件参照可能を再確認しました。
- 運用方針は据え置きです。汎用俯瞰ではなく、各RQを1件ずつ深掘りして「検証命題・応募テーマ・使用EEGデータ」を固定して進めます。

## 2026-03-16 22:48 JST 再検証ログ（本run / 依頼反映）

- 作業前に `main` を `pull --ff-only` で最新化し、基準コミットを `mind-upload=c24de3cff1df`、`EEG-DATA=3dd9f5c3fe4d`、`auto-research-funds=f0d89423aa3f` に更新しました。
- 本ページの `RQ` 行を再計数し、`RQ_TOTAL=60`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を再確認しました。
- 判定内訳は再計数で `A/B/C=17/25/18` を維持し、`mind-upload-rq60-deep-focus-notes.md` / `mind-upload-eeg-rq60-grant-dataset-playbook.md` と整合しました。
- 助成キー `G1-G6`（`GR-2026-013`, `GR-2026-014`, `9Lx4dPK6a4k2gOb7`, `Drbm6vBRDJkn0NGJ`, `871pw3rLjNPKgqA0`, `46z9VPE4wnkrvEJR`）は `grant_queue` と `auto-research-funds/wiki/Mind-Upload-EEG-RQ-Grant-Map.md` で再参照可能でした。
...
## 2026-03-17 04:04 JST 再検証ログ（本run / 1RQ深掘り整合の明示）

- 作業前に `git pull --ff-only origin main` を実行し、`main` 最新状態で再検証しました。
- `research_harvest_50.md` と本ページの `Ux-RQy` 行を再照合し、`RQ_TOTAL=60`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を再確認しました（欠落 `0`、重複 `0`）。
- `auto-research-funds/wiki/Mind-Upload-EEG-RQ-Grant-Map.md` との突合で、`G1-G6` 定義と提出優先キューは一致している一方、RQ本文表の一部で `ID` 参照と `D` 参照にズレがあることを明示しました（対象: `U1-2`, `U7-2`, `U8-2`, `U13-1`, `U14-1`, `U15-2`）。
- 運用を再固定します。申請文・再現ログの正本は `D1-D16 + DOI` とし、RQ本文表の数値 `ID` は「探索候補集合（可否検証側）」として扱います。
- 依頼方針を継続します。汎用俯瞰ではなく `1RQ=1検証命題=1応募テーマ=1主データ` を維持し、各RQを個別に深掘りして提出へ接続します。

### 2026-03-17時点の申請固定データ（ズレ対象6件の正本）

| RQ | 申請で使う主/予備データ（正本） | 助成キー |
|---|---|---|
| U1-2 | `D7` / `D12` | `G4` |
| U7-2 | `D7` / `D4` | `G3` |
| U8-2 | `D10` / `D8` | `G2` |
...
## 2026-03-17 09:04 JST 再検証ログ（本run / 依頼反映: RQごと深掘り運用の固定）

- 作業前に `main` を `pull --rebase --autostash origin main` し、最新状態を確認しました（更新差分なし）。
- `mind-upload/research_harvest_50.md` 正本と本ページの `Ux-RQy` 行を再照合し、`RQ_TOTAL=60`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を再確認しました。
- 判定内訳 `A/B/C=17/25/18` を再計数で確認しました。
- `推奨EEG-DATA` の参照IDは `EEG-DATA/eeg_dataset_summary_ja.csv` で欠落 `0`、助成キー `G1-G6` と追加候補IDは `grant_queue` と `auto-research-funds` で欠落 `0` を確認しました。
- 依頼方針をこのページ正本として再固定します。汎用俯瞰を優先せず、`1RQ=1検証命題=1応募テーマ=1主データ` を維持して1件ずつ深掘りします。

### このrunで固定した深掘り優先キュー（6件）

1. `U14-1`（再現契約の最小粒度）: 主データ `56`、主応募 `G1`（予備 `G3`）
2. `U7-2`（時刻同期誤差監査）: 主データ `6`、主応募 `G1`（予備 `G3`）
3. `U8-2`（オンライン較正とドリフト）: 主データ `696`、主応募 `G2`（予備 `G5`）
4. `U1-2`（不確実性伝播）: 主データ `56`、主応募 `G1`（予備 `G4`）
5. `U13-1`（意味一致と因果一致の分離）: 主データ `509`、主応募 `G1`（予備 `G4`）
6. `U15-2`（neurorights監査写像）: 主データ `49`、主応募 `G2`（予備 `G6`）

## 2026-03-17 10:04 JST 再検証ログ（本run / EEG-DATA更新後のID再固定）
...
- `EEG-DATA/eeg_dataset_summary_ja.csv` の大規模更新後、RQ表が参照する数値ID `24件` のうち現行CSVで一致したのは `1件`（`6`）のみで、`23件`（`11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は再採番により不一致になりました。
...
1. `U14-1`（固定粒度と追試成功率）: `主D12 / 予備D8`、`G1`（予備 `G3`）
2. `U7-2`（時刻同期誤差監査）: `主D7 / 予備D4`、`G1`（予備 `G3`）
3. `U8-2`（オンライン較正とドリフト）: `主D10 / 予備D8`、`G2`（予備 `G5`）
4. `U1-2`（不確実性伝播）: `主D7 / 予備D12`、`G1`（予備 `G4`）
5. `U13-1`（意味一致と因果一致の分離）: `主D2 / 予備D1`、`G1`（予備 `G4`）
6. `U15-2`（neurorights監査項目写像）: `主D4 / 予備D5`、`G2`（予備 `G6`）

## 2026-03-18 15:20 JST 再検証ログ（本run / RQ単位の正本参照を再固定）

- 作業前に `git pull --ff-only origin main` を実行し、`main` 最新を基準に更新しました（差分なし）。
- 正本 `mind-upload/research_harvest_50.md` と本ページ `RQ` 主表を再照合し、`RQ_TOTAL=60`、`A/B/C=17/25/18` を再確認しました。
- `U別の第一応募先/予備応募先` 固定表と、末尾の6件優先キューの助成キー不整合を修正しました（`U14-1/U7-2/U8-2/U1-2/U13-1/U15-2`）。
- `EEG-DATA/eeg_dataset_summary_ja.csv` は再採番で `ID -> データセット名` が変動するため、申請・再現ログの正本を `D1-D16 + DOI + データセット名 + access区分` に再固定しました。

### このrunで固定した6件の深掘りキュー（RQ単位）

| RQ | 主応募 / 予備応募 | 主 / 予備データ（正本） | DOI固定の参照メモ |
|---|---|---|---|
| U14-1 | `G1 / G3` | `D12 / D8` | `D12=10.6084/m9.figshare.13082495`、`D8=10.17632/vyczny2r4w.1` |
| U7-2 | `G1 / G3` | `D7 / D4` | `D7=10.21227/dv1p-vq18`、`D4=10.21227/rnms-3h47` |
| U8-2 | `G2 / G5` | `D10 / D8` | `D10=10.5281/zenodo.14398633`（mirrorあり）、`D8=10.17632/vyczny2r4w.1` |
| U1-2 | `G1 / G4` | `D7 / D12` | `D7=10.21227/dv1p-vq18`、`D12=10.6084/m9.figshare.13082495` |
| U13-1 | `G1 / G4` | `D2 / D1` | DOIは `Mind-Upload-EEG-RQ-Grant-Map` 側の `D1/D2` 正本を優先 |
| U15-2 | `G2 / G6` | `D4 / D5` | `D4=10.21227/rnms-3h47`、`D5` DOIは `Mind-Upload-EEG-RQ-Grant-Map` 正本を優先 |

## 2026-03-17 20:02 JST 再検証ログ（本run / 依頼反映: RQ60総点検）

- 作業前に `main` を `pull --ff-only` で確認し、最新状態から再検証を実施しました。
- 正本 `mind-upload/research_harvest_50.md` の `リサーチクエスチョン分解` を機械再計数し、`RQ_TOTAL=60` を再確認しました。
- 本ページ主表（`## U0 リサーチクエスチョン` から `## 実行優先順` 直前）の `RQ` 行を再計数し、`RQ_TOTAL=60`、`A/B/C=17/25/18` を再確認しました。
- 参照EEGデータ `24 ID`（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は `EEG-DATA/eeg_dataset_summary_ja.csv` で欠落 `0` を確認しました。
- 助成キー `G1-G6` 実ID（`GR-2026-013`, `GR-2026-014`, `9Lx4dPK6a4k2gOb7`, `Drbm6vBRDJkn0NGJ`, `871pw3rLjNPKgqA0`, `46z9VPE4wnkrvEJR`）は `grant_queue` と `auto-research-funds` で全件参照可能でした。
- 方針を再固定します。汎用俯瞰ではなく、`1RQ=1検証命題=1応募テーマ=1主データ` の粒度で1件ずつ深掘りして更新します。

## 2026-03-17 21:02 JST 再検証ログ（本run / 依頼反映: 全60RQを深掘り正本へ同期）

- 作業前に `main` を `pull --ff-only` し、最新状態を確認してから更新しました。
- 本ページを `mind-upload/github-wiki-export/mind-upload-eeg-rq60-feasibility-and-funds.md` と同期し、`1RQ=1検証命題=1応募テーマ=1主データ` の深掘り形式を正本として反映しました。
- `mind-upload/wiki/mind-upload-rq60-deep-focus-notes.md` 主表を再計数し、`RQ_TOTAL=60`、`A/B/C=17/25/18` を再確認しました。
- 参照EEG ID（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は `EEG-DATA/eeg_dataset_summary_ja.csv` で全件実在を再確認しました。
- 助成キー `G1-G6` 実ID（`GR-2026-013`, `GR-2026-014`, `9Lx4dPK6a4k2gOb7`, `Drbm6vBRDJkn0NGJ`, `871pw3rLjNPKgqA0`, `46z9VPE4wnkrvEJR`）は `grant_queue` および `auto-research-funds/grant_eeg_dataset_match.csv` で再参照可能でした。
- 方針を維持します。汎用的な浅い総覧ではなく、各RQを1件ずつ深掘りして申請文に直接転記できる粒度で運用します。

## 2026-03-18 10:40 JST 再検証ログ（本run / EEG-DATA在庫統計を使ったRQ深掘り再固定）

- 作業前に `main` を `pull origin main` し、最新状態を確認しました（更新差分なし）。
- 今回は汎用総覧ではなく、`RQ60` の親問題である `U0/U1/U3/U4/U7/U8/U10/U11/U12/U13/U14/U15` を1件ずつ深掘りし、`EEG-DATA` で検証できる範囲と `auto-research-funds` の応募テーマ接続を再固定しました。
- `EEG-DATA` は「生データ置き場」ではなくメタデータ在庫である点を明示し、在庫規模 `32,088`（高信頼 `28,131`）/ アクセス区分 `immediate=24,956` `account_required=588` `uncertain_public=6,541` を前提値として採用しました。
- RQ可否判定は既存どおり `A/B/C=17/25/18` を維持しつつ、応募設計は `G1-G6` と追加レーン（臨床/AI/高齢社会）に再割当しました。
- 実務運用は継続して `1RQ=1検証命題=1応募テーマ=1主データクラス` とし、各RQを浅く横断せず個別に処理します。

### U別の深掘り再固定（EEG可否 + 応募テーマ + 使うEEGデータクラス）

| U | EEG可否 | 深掘りで固定した検証焦点（RQ単位で共通化するKPI） | 主応募テーマ（auto-research-funds） | 主に使うEEGデータクラス |
|---|---|---|---|---|
| U0 操作的同一性 | `A/B` | `介入一致率` `反事実一致率` を個体内/個体間で分離し、分岐ケースは制度連携へ送る。 | `G1`（技術実証）/ `G2`（統合計画） | cross-session EEG biometrics, longitudinal MI-BCI, multimodal identity proxies |
| U1 逆問題同定可能性 | `A/B` | `位置誤差(mm)` `不確実性幅` を必須化し、逆解法間乖離の失敗閾値を事前登録。 | `G1` / `G4` | high-density scalp EEG, intracranial-ground-truth, EEG-fMRI同時計測 |
| U3 生物学的境界 | `B/C` | EEG単独 vs multimodal拡張で `ΔR2` を比較し、glymphatic/免疫は外部依存として明示。 | `G2` / `G6` | multimodal EEG (EEG-fMRI/fNIRS), naturalistic & continuous paradigms |
| U4 因果同値 | `A/B` | 介入有無で `因果辺再現率` `符号反転率` を測り、反証条件を明文化。 | `G1` / `G4` | stimulation-ready EEG (TMS/rTMS/tES), intervention cohorts, known-source setups |
| U7 マルチモーダル整合 | `A` | `同期誤差(ms)` `再現率低下(pt)` をCIで監査し、QC欠落を不採択化。 | `G1` / `G3` | EEG-fMRI/EEG-fNIRS, benchmark/standardized pipelines, timing audit datasets |
| U8 閉ループ安定性 | `B` | `安定率` `停止介入率` `回復時間` を同時評価し、再学習頻度の最適化へ接続。 | `G2` / `G5` | closed-loop neurofeedback, longitudinal BCI, EMG/TMS-guided control datasets |
| U10 熱力学的一貫性 | `C` | 直接熱計測は不可として `理論整合スコア` `proxy相関` までに主張を制限。 | `G2` / `G6` | sedation/sleep-state EEG, EEG-fNIRS/PPG/ECG併用proxy datasets |
| U11 意識指標近似 | `B/C` | `理論間順位一致率` `偽陽性率` を統一仕様で比較し、理論最終裁定は留保。 | `G2` / `G4` + 臨床レーン | sleep/anesthesia/DoC cohorts, clinical outcome-labeled EEG |
| U12 分岐本人性 | `C` | EEGは `監査ログ完全性` の補助証拠に限定し、責任/権利は制度側で判断。 | `G2` / `G6` | longitudinal identity-tracking EEG, branch-condition audit logs |
| U13 模倣分離 | `B` | `意味一致率` と `機構識別率` を二軸化し、shortcut検出を必須項目に固定。 | `G1` / `G4` + AIレーン | speech/reading decoding EEG, overt-vs-imagined contrasts, language multimodal datasets |
| U14 追試可能性 | `A/B` | `追試成功率` `再現コスト増分` を運用KPI化し、否定結果公開を契約へ組込む。 | `G1` / `G3` | benchmark-ready datasets, leakage-auditable splits, reproducibility-carded resources |
| U15 制度統合 | `C` | `規制要件充足率` `停止基準発動時間` を定義し、EEGは安全監査ログ基盤として利用。 | `G2` / `G6` + 高齢・地域レーン | safety-stop related EEG, fatigue/sleep/rehab monitoring datasets |

### 助成テーマ選定ルール（今回runで再固定）

- 第1優先: `Mind-Upload-EEG-RQ-Grant-Map` 本文で定義済みの `G1-G6`（キュレーション済み）。
- 第2優先: `ARF-Cards-Index-*` の締切近接と `脳波・神経近接` スコアを使った運用候補。
- 第3優先: `grant_eeg_dataset_match.csv` は探索補助としてのみ使用（最終決定の正本にはしない）。
- 申請本文には、数値IDではなく `DOI + データセット名 + access区分(immediate/account_required/uncertain_public)` を必須記載する。

### このrunで次回に引き継ぐ実行順（RQ単位）

1. `U14-1` 再現契約最小粒度（`G1/G3`、benchmark系EEG）
2. `U7-2` 同期誤差監査（`G1/G3`、timing audit系EEG）
3. `U8-2` オンライン較正・ドリフト（`G2/G5`、closed-loop系EEG）
4. `U1-2` 不確実性伝播（`G1/G4`、high-density/ground-truth系EEG）
5. `U13-1` 模倣分離（`G1/G4`、language/speech EEG）
6. `U15-2` neurorights監査写像（`G2/G6`、safety monitoring EEG）

## 2026-03-18 09:31 JST 再検証ログ（本run / 申請正本運用の再確認）

- `mind-upload` 側の `RQ60` 表（`feasibility` / `playbook` / `deep-focus`）を突合し、`RQ_TOTAL=60`、`A/B/C=17/25/18` の整合を再確認。
- 参照EEGデータID `24件` は `EEG-DATA/eeg_dataset_summary_ja.csv` で全件実在（欠落 `0`）。
- 助成実ID `G1-G6`（`GR-2026-013`, `GR-2026-014`, `9Lx4dPK6a4k2gOb7`, `Drbm6vBRDJkn0NGJ`, `871pw3rLjNPKgqA0`, `46z9VPE4wnkrvEJR`）の参照可能性を再確認。
- 申請実務の正本は継続して `D1-D16 + DOI + データセット名 + access区分` を必須とし、数値IDは探索補助に限定。
- 運用方針は継続: `1RQ=1検証命題=1応募テーマ=1主データ` の深掘り単位で提出判断を行う。

## 2026-03-18 14:20 JST 再検証ログ（本run / 依頼反映: 60RQを1件ずつ深掘りする運用で再固定）

- 作業前に `main` を `pull --ff-only` し、最新状態で再検証しました。
- `mind-upload/wiki/mind-upload-eeg-rq60-feasibility-and-funds.md` の RQ表を機械再計数し、`RQ_TOTAL=60`、`A/B/C=17/25/18` を再確認しました（欠落 `0`、重複 `0`）。
- 本ページが参照する EEG 数値ID `17件`（`6, 11, 13, 16, 19, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 2412`）は `EEG-DATA/eeg_dataset_summary_ja.csv` で全件実在（欠落 `0`）を再確認しました。
...
## 2026-03-18 08:04 JST 再検証ログ（本run / 依頼反映: 全60RQの深掘り運用を再固定）

- 作業前に `auto-startup` / `mind-upload` / `auto-research-funds` / `EEG-DATA` の各 `main` を `pull --ff-only` で更新し、最新状態で再検証しました。
- `mind-upload/wiki/mind-upload-eeg-rq60-feasibility-and-funds.md` の `Ux-RQy` 一意キーは `60` 件（欠落 `0` / 重複 `0`）を再確認しました。
- 判定内訳 `A/B/C=17/25/18` を再計数し、既存の判断配分と一致することを確認しました。
...
## 2026-03-18 10:18 JST 再検証ログ（本run / RQ60深掘り運用の更新）

- `research_harvest_50.md` を正本として再照合し、`RQ_TOTAL=60`（`U0=4 U1=4 U3=6 U4=4 U7=6 U8=6 U10=4 U11=4 U12=6 U13=6 U14=6 U15=4`）を再確認しました。
- 判定内訳 `A/B/C=17/25/18` を再計数し、本ページのRQ行と一致しました。
- 本runで参照したEEGデータID（`6, 11, 13, 16, 19, 29, 39, 49, 56, 65, 509, 676, 696, 719, 735, 783, 842, 859, 1011, 1839, 1972, 2412, 3419, 4878`）は `EEG-DATA/eeg_dataset_summary_ja.csv` で全件実在（欠落 `0`）を確認しました。
- 助成実ID `G1-G6`（`GR-2026-013`, `GR-2026-014`, `9Lx4dPK6a4k2gOb7`, `Drbm6vBRDJkn0NGJ`, `871pw3rLjNPKgqA0`, `46z9VPE4wnkrvEJR`）は `grant_queue` と `auto-research-funds/wiki/Mind-Upload-EEG-RQ-Grant-Map.md` で再参照可能でした。
- 方針を継続します。汎用俯瞰ではなく `1RQ=1検証命題=1応募テーマ=1主データ` の深掘り運用で扱います。

### 次に1件ずつ深掘りする固定順（本run更新）

1. `U4-2` 最小介入因果主張の再現可否（主データ `56` / 応募 `G1`）
2. `U7-5` 前処理CI release block閾値の固定（主データ `676` / 応募 `G1`）
3. `U8-4` 異常検知遅延とフェイルセーフ発動率（主データ `2412` / 応募 `G2`）
