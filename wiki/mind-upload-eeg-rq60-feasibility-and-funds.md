...
- 更新日: 2026-04-02 / 位置づけ: RQ-by-RQ Deep Dive
...
## Latest post-pull revalidation after 22:06 JST on April 4, 2026

This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@39248d59caaa`, `EEG-DATA@7ba2201e2b5`, and `auto-research-funds@f90279417a9f` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` adds paired-fluid barrier route guidance outside the current six, `EEG-DATA` deepens the `IEEE DataPort EEG records` audit while now keeping `30,935` merged-unique rows, `28,898` high-confidence rows, `24,910` directly usable rows, and `24,540` high-confidence immediate rows, and `auto-research-funds` only relaxes moving-head references without changing the practical shortlist.
...
- The route-safe access reading also stays deliberately narrow in one sharper way: `D03 / D02 / D01` still look more open on DOI/DataCite/catalog-side reads than they do on the actual IEEE DataPort landing pages, so `D10` remains the only clear public-open primary anchor while `D03 / D02 / D01` still stay gated-support anchors in public wording.
- The newest repo-wide carry-forward still does not create a stronger row-level replacement: the paired-fluid barrier note improves outside-current-six bridge language, the newest IEEE DataPort audit improves access-status discipline, and the funding wording cleanup keeps the shortlist easier to reuse, but none of them repairs the recall-validity gap under `U13-5`, replaces the cross-session identity-drift floor under `U0-2 / U0-3`, or displaces the current `D11 / D08 / D05 / D20` outside-current-six family.
- Browser-level live checks with a browser user agent still returned `HTTP/2 200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, `wiki/mind-upload-eeg-rq60-grant-dataset-playbook.html`, `wiki/u13-2-brain-minus-prior-route.html`, and `wiki/u13-5-perception-to-recall-route.html`; the current `Last-Modified` header is `Sat, 04 Apr 2026 12:16:08 GMT` on those public surfaces, while the checked bodies still surfaced the earlier `2026-04-04 17:07 JST` wording on the front door plus `U13-2`, and `2026-04-04 19:09 JST` wording on `U13-5`, before this source sync.
...
- A fresh read-only Todoist `api/v1/sync(items, projects)` full-item audit still showed enough route coverage (`items=207`, `projects=2`), so no new task was added in this session because the current route and watch route were already covered and no new direct-fit EEG-first lane displaced them.
...
- Recounting `rq_deepening_backlog_2026-03-01_enriched.csv` still returns `60` rows, so this run still keeps the `one-RQ-at-a-time` reading rather than flattening the map.
- The route-safe access reading also stays deliberately narrow: `D10` remains the only clear public-open primary anchor, while `D03 / D02 / D01` still stay gated-support anchors in public wording because no newest pulled head turns those three routes into immediate public anchors.
- The newest repo-wide carry-forward still does not create a stronger row-level replacement: `Numbers` and `PRIOS` improve support-space specificity, but they do not repair the recall-validity gap under `U13-5`, replace the cross-session identity-drift floor under `U0-2 / U0-3`, or displace the current `D11 / D08 / D05 / D20` outside-current-six family.
- Browser-level live checks with a browser user agent still returned `HTTP/2 200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, and `wiki/u13-2-brain-minus-prior-route.html`; the current `Last-Modified` header is `Fri, 03 Apr 2026 15:13:06 GMT` on those public surfaces, while the checked bodies still surfaced the earlier `2026-04-04 00:06 JST` wording before this source sync.
- Official-window spot checks keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Kashinome Scholarship = 2026-01-14 - 2026-04-09 (must arrive)`, `Nakatani Foundation research grant = closed / next-cycle watch`, and `Secom General Research Grant = current cycle closed`.
- A fresh read-only Todoist `api/v1/sync(items, projects)` full-item audit still showed enough route coverage (`items=206`, `projects=2`, `Kura=2`, `Nakatani family=11`, `Kashinome=2`, `Secom=3`, `Brain Science Foundation=8`, `Okawa=2`, `Japan Industrial Science Research Institute=1`, `Keio graduate scholarship=2`, `G-7=1`, `ENEOS=1`, `同盟会=0`, `研究のすゝめ=0`), so no new task was added in this session because the current route and watch route were already covered and the two newly covered scholarships still stay outside the present EEG-first route.

## Latest post-pull revalidation after 05:12 JST on April 3, 2026

This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@a9fc5d1cb691f3669c40803604e78cf87fab4eed`, `EEG-DATA@8c3ace28bad0b3bac259caa440994e5d05bfc7d1`, and `auto-research-funds@35756f8381c47daa24bb70bb7a1ff88d1dcf76bc` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` is now at a `docs: sync github wiki export` head that keeps the exported route mirrors aligned without changing the active queue, `EEG-DATA` carries forward a `VR navigation dataset` output refresh on top of a recent `EESM17` metadata deepening while still keeping `30,900` merged-unique rows, `28,863` high-confidence rows, and `24,885` directly usable rows, and `auto-research-funds` adds `JSPS overseas fellow` current-card snapshots on top of a recent `JSSF research publication` refresh while now reporting `8,594` aid IDs, `4,307` rows, and `3,957` official-source rows without changing the practical shortlist.
- Recounting `rq_deepening_backlog_2026-03-01_enriched.csv` still returns `60` rows, so this run still keeps the `one-RQ-at-a-time` reading rather than flattening the map.
- The route-safe access reading also stays deliberately narrow: `D10` remains the only clear public-open primary anchor, while `D03 / D02 / D01` still stay gated-support anchors in public wording because no newest pulled head turns those three routes into immediate public anchors.
...
- Browser-level live checks with a browser user agent still returned `HTTP/2 200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, and `wiki/u13-2-brain-minus-prior-route.html`; the current `Last-Modified` header is `Thu, 02 Apr 2026 19:17:56 GMT` on those public surfaces, while the checked bodies still surfaced the earlier `04:05/04:06 JST` hold note before this source sync.
- Official-window spot checks keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Kashinome Scholarship = 2026-01-14 - 2026-04-09 (must arrive)`, `Nakatani Foundation research grant = closed / next-cycle watch`, and `Secom General Research Grant = current cycle closed`.
...
This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@85b6603b569b`, `EEG-DATA@a7f9390492fd`, and `auto-research-funds@b6c0d0f4eeb7` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` only syncs `U7-2` public bridge wording, `EEG-DATA` deep-dives `OSF gazx2` confidence EEG metadata and `OpenNeuro ds007081` while now reporting `30,892` merged-unique rows, `28,855` high-confidence rows, and `24,880` directly usable rows in the current README snapshot, and `auto-research-funds` refreshes its README summary to `8,590` aid IDs, `4,301` rows, and `3,951` official-source rows.
- Recounting `rq_deepening_backlog_2026-03-01_enriched.csv` still returns `60` rows, so this run still keeps the `one-RQ-at-a-time` reading rather than flattening the map.
- The route-safe access reading also stays deliberately narrow: `D10` remains the only clear public-open primary anchor, while `D03 / D02 / D01` still stay gated-support anchors in public wording because a direct live-access reread still shows `IEEE DataPort` login and subscription gates on those three routes.
- The newest repo-wide carry-forward does not create a stronger row-level replacement: the speech-side backup family is still useful, but nothing in the latest pulled heads repairs the recall-validity gap under `U13-5` or replaces the cross-session identity-drift floor under `U0-2 / U0-3`.
- Browser-level live checks with a browser user agent still returned `HTTP/2 200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, and `wiki/u7-2-timing-metric-route.html`; the current `Last-Modified` header is `Thu, 02 Apr 2026 08:22:50 GMT` on those public surfaces.
- Official-window spot checks keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Kashinome Scholarship = 2026-01-14 - 2026-04-09 (must arrive)`, `Nakatani Foundation research grant = closed / next-cycle watch`, and `Secom General Research Grant = current cycle closed`.
- A fresh read-only Todoist `api/v1/sync(items, projects)` full-item audit still showed enough route coverage (`items=203`, `projects=2`, `Kura=2`, `Nakatani family=11`, `Nakatani graduate scholarship=7`, `Kashinome=2`, `Secom=3`, `Brain Science Foundation=8`, `Okawa=2`, `Japan Industrial Science Research Institute=1`, `AI for Aging-Society Problem Solving=0`), so no new task was added in this session because the current route and watch route were already covered and the aging-society lane still stays outside the present EEG-first route.

## Latest post-pull revalidation after 10:07 JST on April 2, 2026

This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@be507c810b11`, `EEG-DATA@3f20f48cca6e`, and `auto-research-funds@fb32c05163f4` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` refines EEG foundation setup-equivalence boundaries outside the current six, `EEG-DATA` now reports `30,887` merged-unique rows, `28,850` high-confidence rows, and `24,871` directly usable rows after the HBN EEG release 11 deep dive, and `auto-research-funds` keeps the same practical shortlist while its repository stats still read `8,585` aid IDs, `4,293` rows, and `3,943` official-source rows.
- Recounting `rq_deepening_backlog_2026-03-01_enriched.csv` still returns `60` rows, so this run still keeps the `one-RQ-at-a-time` reading rather than flattening the map.
- The route-safe access reading also stays deliberately narrow: `D10` remains the only clear public-open primary anchor, while `D03 / D02 / D01` still stay gated-support anchors in public wording because a direct live-access reread still shows `IEEE DataPort` login and subscription gates on those three routes.
- The current row-by-row source of truth also becomes tighter in this run: the public-side deep dossier and the funding-facing deep dossier are re-synchronized to the same `Dxx + DOI + dataset name + access class` wording, so the `A/B/C` map and the grant-facing rows now point to the same canonical packets again.
- Browser-level live checks with a browser user agent still returned `200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, and `wiki/u8-1-closed-loop-delay-tolerance-route.html`; the current `Last-Modified` header is `Thu, 02 Apr 2026 00:15:06 GMT` on those public surfaces.
- Official-window spot checks keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Kashinome Scholarship = annual mid-January to early-April window on the official page`, `Nakatani Foundation research grant = closed / next-cycle watch`, and `Secom General Research Grant = current cycle closed`.
...
## Latest post-pull revalidation after 09:05 JST on April 2, 2026

This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@d7f30648fc40`, `EEG-DATA@ac8cc6fc8e74`, and `auto-research-funds@31ec17305f6a` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` tightens a literature myelin-route split outside the current six, `EEG-DATA` now reports `merged_unique=30886`, `high_confidence=28849`, and `access_counts_high.immediate=24500`, and `auto-research-funds` carries forward the `Aoi graduate scholarship` / `JSSF research grant` merges plus a README-stat refresh to `8,585` aid IDs, `4,293` rows, and `3,943` official-source rows without displacing the practical shortlist.
- Recounting `rq_deepening_backlog_2026-03-01_enriched.csv` still returns `60` rows, so this run still keeps the `one-RQ-at-a-time` reading rather than flattening the map.
- The route-safe access reading also stays deliberately narrow: `D10` remains the only clear public-open primary anchor, while `D03 / D02 / D01` still stay gated-support anchors in public wording because a direct live-access reread still shows `IEEE DataPort` login and subscription gates on those three routes.
- Browser-level live checks with a browser user agent still returned `200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, and `wiki/u8-1-closed-loop-delay-tolerance-route.html`; the current `Last-Modified` header is `Wed, 01 Apr 2026 23:29:12 GMT` on those public surfaces, and the live pages still surfaced the earlier `04:09 JST` / `03:08 JST` reread notes before this source update.
- Official-window spot checks keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Kashinome Scholarship = annual mid-January to early-April window on the official page`, `Nakatani Foundation research grant = closed / next-cycle watch`, and `Secom General Research Grant = current cycle closed`.
- A fresh read-only Todoist `api/v1/sync(items, projects)` full-item audit still showed enough route coverage (`items=203`, `projects=2`, `Kura=2`, `Nakatani family=11`, `Nakatani graduate scholarship=7`, `Kashinome=2`, `Secom=3`, `Brain Science Foundation=8`, `Okawa=2`, `Japan Industrial Science Research Institute=1`, `AI for Aging-Society Problem Solving=0`), so no new task was added in this session.

## Latest post-pull revalidation after 04:09 JST on April 2, 2026

This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@92d1547bfeb9`, `EEG-DATA@2e3799a174d9`, and `auto-research-funds@f5f55bcbf724` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` splits the beginner molecular-maintenance family outside the current six, `EEG-DATA` now reports `merged_unique=30877`, `high_confidence=28840`, and `access_counts_high.immediate=24492`, and `auto-research-funds` carries forward the large April 2 funding refresh without displacing the practical shortlist.
- Recounting `rq_deepening_backlog_2026-03-01_enriched.csv` still returns `60` rows, so this run still keeps the `one-RQ-at-a-time` reading rather than flattening the map.
- The route-safe access reading also stays deliberately narrow: `D10` remains the only clear public-open primary anchor, while `D03 / D02 / D01` still stay gated-support anchors in public wording.
- Browser-level live checks with a browser user agent still returned `200` for `issue.html`, `wiki/mind-upload-current-public-six-rq-brief.html`, `wiki/mind-upload-rq-solvability-bridge.html`, and `wiki/u8-1-closed-loop-delay-tolerance-route.html`; the current `Last-Modified` header is `Wed, 01 Apr 2026 18:24:31 GMT` on those public surfaces.
- Official-window spot checks keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Nakatani Foundation research grant = closed / next-cycle watch`, `Secom General Research Grant = closed`, and `Kashinome Scholarship = support-only / repo-side current card keeps 2026-04-09`.
- A fresh read-only Todoist `api/v1/sync(items, projects)` full-item audit still showed enough route coverage (`items=203`, `projects=2`, `Kura=2`, `Nakatani family=11`, `Nakatani graduate scholarship=7`, `Kashinome=2`, `Secom=3`, `Brain Science Foundation=8`, `Okawa=2`, `Japan Industrial Science Research Institute=1`, `AI for Aging-Society Problem Solving=0`), so no new task was added in this session.

## Latest post-pull revalidation after 17:06 JST on April 1, 2026

This run still does not change the current public batch or the `A/B/C` reading. An actual-pull reread after `mind-upload@c5a0550`, `EEG-DATA@82893f6ca6e`, and `auto-research-funds@ec6f7d068` still leaves the current `D10 / D03 / D02 / D01` family and the outside-current-six order `U7-1 -> U1-2 -> U4-2 -> U8-1` unchanged.

- The newest pulled deltas still deepen support rather than replace the current route: `mind-upload` deepens astrocyte PET route splits outside the current six, `EEG-DATA` refreshes the KMI EEG deep-dive and now reports `merged_unique=30871`, `high_confidence=28834`, and `access_counts_high.immediate=24484`, its experiment-metadata report still covers `18,312 / 28,832` rows with at least one experiment-facing field (`63.51%` coverage), and `auto-research-funds` adds the `Megachips 2026 scholarship` snapshot plus a README-stat refresh without changing the practical shortlist.
