---
layout: default
title: 'Wiki: ベースライン / ベンチマーク / 事前登録 / モデルカード'
description: スコアが比較可能な証拠と混同されないように、ベースライン、ベンチマーク、事前登録、モデル カード、および現在のクレームトリガー コンパニオン、ルート固有、およびフィールド レベルのカード要件を整理します。
article_type: ウィキ
subtitle: スコアレポートからクレームに必要な現在のアーティファクトスタックまで
author: マインドアップロード研究プロジェクト
last_updated: '2026-04-04'
note: 学習ガイド / 現在のカードフィールドの同期
audience: ベースライン、ベンチマーク、事前登録、およびモデルカードの言語が似ていると感じる人、およびより強力な主張が許可される前にどの追加カードが必要かを知りたい人
reading_time: 12～18分
page_intro: このページでは、ベースライン、ベンチマーク、事前登録、モデル カード、および結果がマルチモーダル フュージョン、大規模な事前トレーニング、ショートカット耐性、言語対応デコード、ルート固有の測定/推論クレーム、生きている人間のプロキシ
  バンドル、または連続した同一サブジェクト ブリッジに依存する場合に必要となる追加カードの操作上の違いについて説明します。今回の改訂では、カード名だけでは大まかになりすぎた部分と、フィールドレベルの開示が必要になった部分も示しています。
accuracy_note: このページは、アーティファクト スタックの学習ガイドです。権限のあるカード フィールドと停止ルールは公開検証ページにまだ存在します。
page_highlights:
- ベンチマークは単なるスコアシートではありません。このサイトには、分割ルール、メトリック セマンティクス、ベンチマーク ガバナンスも含まれています。
- 通常のモデル カードでは、すべてのクレーム形状に十分ではありません。一部の結果にはクレームトリガーのコンパニオン カードが必要ですが、一部の結果にはルート固有のカードまたはログも必要になります。
- デコードまたは表現のクレームの場合、ショートカット耐性はスコア レポートとは別の監査となります。
- 言語対応のテキスト/音声出力の場合、ニューラル貢献カードは一般的なショートカット監査とは別に扱われるようになりました。
- マルチモーダルまたはアトラス以前のクレームの場合、Fusion Card は同期ミドルウェアや共同登録から分離されており、有効ウィンドウの不一致、共有要素と固有要素、数量ブリッジ、およびバンドルの堅牢性も分離する必要があります。
- ESI、トラクトグラフィー、有効接続性、および熱力学的主張について、現在のサイト規則では、汎用モデル カードのみではなく、ルート固有の開示を求めています。
- 複数の生きた人間のプロキシ行を一緒に使用する場合、同一主体の状態識別言語を許可する前に人間プロキシ構成カードが必要です。また、行ごとの役割、体制の互換性、運用の成熟度、キャリブレーターの役割、不一致トポロジーも開示する必要があります。
- 同一主題または同一脳の逐次ブリッジの場合、同一状態言語を許可する前に状態継続ブリッジ カードが必要です。また、運ばれるオブジェクト、許容/失敗ルール、および救済ルートの名前も指定する必要があります。
known_points:
...
- このサイトでは、カード名だけではもはや十分ではありません。Fusion、Human Proxy Composition、および State-Continuity Bridge の各カードでは、新しい形式のオーバーリードをブロックするためにフィールド
  レベルの開示が必要になりました。
...
- label: 'Wiki: 検証の基本'
  ...
  description: 標準、ベンチマーク、レジストリ、監査が必要な理由のより広い地図。
- label: 'Wiki: データセットの分割と漏洩'
  url: /wiki/dataset-splits-and-leakage.html
  description: 分割の衛生管理、取得と配布のショートカット、ベンチマークの来歴について説明します。
- label: 'Wiki: EEG 基礎モデル'
  url: /wiki/eeg-foundation-models.html
  description: 事前トレーニング クレームに汎用モデル カード以上のものが必要な理由を説明します。
- label: 'Wiki: 人間の代理構成'
  url: /wiki/human-proxy-composition.html
  description: 複数の生きている人間のプロキシ行が一緒に昇格される場合とそうでない場合について説明します。
- label: 'Wiki: 観察から推定へ'
  url: /wiki/observation-to-estimation.html
  description: 逆問題、ESI、効果的な接続性、および因果関係の同等性の制限を整理します。
- label: 'Wiki: 不確実性、校正、および棄権'
  url: /wiki/uncertainty-confidence-and-abstention.html
  description: 確率、予測セット、および棄権が独自のカードを必要とする場合について説明します。
- label: 'Wiki: 熱力学的接地の基本'
  url: /wiki/thermodynamic-grounding-basics.html
  description: 不可逆性言語に信号ルート、推定量ファミリー、数量タイプの開示が必要な理由を説明します。
- label: 'Wiki: 閉ループ、レイテンシー、ジッター、および安全停止'
  url: /wiki/closed-loop-latency-jitter-and-safety-stops.html
  description: 介入のタイミングと身体/環境の境界が別個の監査である理由を説明します。
- label: 'Wiki: 状態継続ブリッジ'
  url: /wiki/state-continuity-bridge.html
  description: 同じ主題または同じ脳の文言が自動的に同じ状態の証拠を意味しない理由を説明します。
recommended_pages:
- label: 検証
...
<p>
L1 以上の結果の場合、このサイトは通常のモデル カードの上に <a href="../verification.html#observability-budget">Observability Budget</a> をスタックするため、測定スタック、直接観測可能量、残留潜在状態、請求上限、および棄権条件がスコアによって暗示されるのではなく可視化されます。
</p>
</div>
...
<h2 class="section-title">まずは役割を分ける</h2>
...
<h2 class="section-title">このサイトのベンチマークで修正される内容</h2>
<table class="data-table">
<thead>
<tr>
<th>ベンチマークフィールド</th>
...
<h2 class="section-title">クレームによってどの追加アーティファクトがトリガーされるか</h2>
<table class="data-table">
<thead>
<tr>
<th>クレーム形状</th>
...
<h2 class="section-title">短いラベルが弱くなりすぎた3枚のカード</h2>
<p>
...
<h2 class="section-title">ルート固有のカードを追加する必要があった理由</h2>
<table class="data-table">
<thead>
<tr>
<th>クレームファミリー</th>
...
<h2 class="section-title">スタックが累積される理由</h2>
<table class="data-table">
<thead>
<tr>
<th>これが欠けている場合</th>
...
<h2 class="section-title">最小限の流れとして見る</h2>
...
<h2 class="section-title">失敗例がスタックに残る理由</h2>
<p>
...
<h2 class="section-title">公開ページを読み取る際の最小限のチェック</h2>
...
<p>
権威のあるカードフィールドについては <a href="../verification.html">Verification</a> に、実践的な実装については <a href="../datasets.html#l0-practice">Datasets / L0 Practice</a> に、クロスドメインの判例については <a href="../verification.html#casework">Verification Casework</a> に戻ってください。
</p>
</section>
...
<li><a href="eeg-foundation-models.html">EEG 基礎モデル -></a></li>
...
<li><a href="closed-loop-latency-jitter-and-safety-stops.html">閉ループ、レイテンシー、ジッター、安全停止 -></a></li>
<li><a href="human-proxy-composition.html">人間プロキシ構成 -></a></li>
