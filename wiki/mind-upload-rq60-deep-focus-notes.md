...
## Latest post-pull hold (2026-04-03 22:04 JST)

- An actual-pull reread after `mind-upload@1ab35b64796f3936e525befb5d9428d789a16263`, `EEG-DATA@be3d03d1c286dcd37fb056317a693e47c39f7f3a`, and `auto-research-funds@845e6849a6c36e6e83394e32cf8dc4b5fe6fdc07` still leaves the current public six-question batch, the solve-first order, and the `A/B/C` reading unchanged.
- The newest pulled deltas still widen support rather than the current practical batch: `mind-upload` now just carries the earlier `21:05 JST` route-hold sync, `EEG-DATA` adds `three more deep EEG dataset overrides` while now reporting `merged_unique=30918`, `high_confidence=28881`, and `access_counts_high.immediate=24528`, and `auto-research-funds` now just carries the earlier `21:05 JST` funding addendum without changing the practical shortlist.
...
- Browser-level live checks at `2026-04-03 22:04 JST` still returned `200` for `issue.html`, `mind-upload-current-public-six-rq-brief.html`, `mind-upload-rq-solvability-bridge.html`, and `u13-2-brain-minus-prior-route.html`; those public surfaces reported `Last-Modified: Fri, 03 Apr 2026 12:24:26 GMT`, and their route-facing bodies already surfaced the earlier `21:05 JST` hold note before this source sync.
...
- A fresh read-only Todoist `api/v1/sync(items, projects)` audit still showed enough route coverage (`items=205`, `projects=2`, `Kura=2`, `Nakatani family=11`, `Nakatani graduate scholarship=7`, `Kashinome=2`, `Secom=3`, `Brain Science Foundation=8`, `Okawa=2`, `Japan Industrial Science Research Institute=1`, `Keio graduate scholarship=2`), so no new task was added in this session.
...
- The newest pulled deltas still widen the support map rather than the current practical batch: `mind-upload` deepens astrocyte PET route splits outside the current six, `EEG-DATA` now reports `merged_unique=30871`, `high_confidence=28834`, and `access_counts_high.immediate=24484`, and its experiment-metadata report still covers `18,312 / 28,832` rows with at least one experiment-facing field (`63.51%` coverage), but those gains still do not replace `D10 / D03 / D02 / D01`.
- Recounting the `### Ux-y` headings in the RQ-by-RQ dossiers still returns `60`, the grant/dataset playbook still keeps `60` unique `Ux-y` rows, and the present practical reading still stays `deepen one row at a time`.
...
- Browser-level live checks at `2026-04-01 17:06 JST` still returned `200` for `issue.html`, `mind-upload-current-public-six-rq-brief.html`, `mind-upload-rq-solvability-bridge.html`, and `u8-1-closed-loop-delay-tolerance-route.html`, and those public surfaces now share `Last-Modified: Wed, 01 Apr 2026 07:16:09 GMT`.
- Official window spot checks still keep the same practical bridge: `Kura Fund = 2026-02-02 - 2026-04-17 17:00`, `Nakatani Foundation graduate scholarship = 2026-04-01 - 2026-05-25 15:00`, `Kashinome Scholarship = 2026-01-14 - 2026-04-09`, while `Nakatani Foundation research grant`, `Secom General Research Grant`, and `Brain Science Foundation research grant` remain outside the current-cycle lane.
...
- The newest pulled deltas still widen the support map rather than the current practical batch: `mind-upload` split temporal routes for invasive language BCIs outside the current six, `EEG-DATA` added a semantic-text-relevance deep dive that widens language-related backup coverage without displacing `D10 / D03 / D02 / D01`, and `auto-research-funds` refreshed `Obayashi / Yokogaku` cards without changing the current shortlist.
- Recounting the `### Ux-y` headings in the RQ-by-RQ dossiers still returns `60`, the grant/dataset playbook still keeps `60` unique `Ux-y` rows, and the present practical reading still stays `deepen one row at a time`.
...
- Anchor strength also stays conservative after the pull. `D10 3M-CPSEED` remains the only clear public-open primary anchor, `D02 CSTE` remains the narrowest gated planning anchor for the identity rows, and `D03 Multimodal Fusion EEG + Eye-Tracking` plus `D01 Acquisition delay of wireless EEG` remain bounded-support anchors with lower claim ceilings.
...
| `5` | `U14-4` | Card requirements should be frozen only after the speech and identity rows stabilize what must actually be disclosed. | `D01 + D13` / `10.21227/dv1p-vq18 + 10.17632/vyczny2r4w` / `gated floor + immediate support` | `Filled Model Card / Dataset Card examples for the current public batch` | `Kura Fund` / `Nakatani Foundation research grant (watch)` |
| `6` | `U14-2` | The exploration/confirmation split is the last step because it depends on the artifact family above being stable enough to rerun without ad hoc reinterpretation. | `D01 + D13` / `10.21227/dv1p-vq18 + 10.17632/vyczny2r4w` / `gated floor + immediate support` | `Fixed-split rerun contract and negative-case trail` | `Kura Fund` / `Nakatani Foundation research grant (watch)` |
...
- Anchor strength also remains unchanged after the pull. `D10` stays the only clear public-open primary anchor for first-pass artifacts, `D02` stays the narrowest gated planning anchor for the identity rows, and `D03` plus `D01` remain bounded-support anchors whose outputs should keep a narrower claim ceiling.
...
| U13-2 | B | Correlation between hallucination test results and neural state differences | Neural decoding connections for hallucination tests (G1/G4) | D10 3M-CPSEED (`10.18112/openneuro.ds006465.v2.0.0`, immediate); adjunct: spoken/imaged robot-speech EEG, D03 | LLM evaluation platform collaboration |
...
| U13-5 | B | Degradation branch point estimation from perception to recall | Perception/recall decoder branch analysis (G1/G4) | D03 Multimodal Fusion EEG + Eye-Tracking (`10.21227/6106-6120`, subscription-gated support); adjunct: D10, ZuCo 2.0, spoken/imaged robot-speech EEG | Unification of task design |
...
| U14-2 | A | Recall rate difference when separating exploration/verification | Operational separation of exploration and verification (G1/G3) | D01 Acquisition delay of wireless EEG (`10.21227/dv1p-vq18`, subscription-gated support); required support: D13; adjunct: D08 | Research management process |
...
| U14-4 | A | Differences in ease of auditing with and without Card description | Model/Dataset Card contracting (G1/G3) | D01 Acquisition delay of wireless EEG (`10.21227/dv1p-vq18`, subscription-gated support); required support: D13; adjunct: D08 | Legal preparation for evaluation contracts |
