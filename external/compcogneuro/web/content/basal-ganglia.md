---
layout: default
title: "compcogneuro/web: basal-ganglia"
description: "compcogneuro/web: basal-ganglia.md の日本語訳"
article_type: 翻訳資料
...
<p>原典: <a href="https://github.com/compcogneuro/web/blob/main/content/basal-ganglia.md" target="_blank" rel="noopener noreferrer">https://github.com/compcogneuro/web/blob/main/content/basal-ganglia.md</a></p>
...
bibfile = "ccnlab.json"
+++
**大脳基底核** (BG) は、[[anatomy|brain]] ([[#figure_bg-anatomy]]) の中心「コア」に位置する核の集合体で、[[neocortex]] の最高レベルの処理と脳幹 (中脳と後脳、脊髄まで続く) の運動制御ネットワークの間の機能的な橋渡しを形成します。 BG の主な出力はこれらの運動ネットワークに下向きにあり、視床への広範な接続を介して新皮質の前頭領域、主に [[prefrontal cortex]] に戻ります。 **線条体**はBGの入力核であり、解剖学的に尾状核と被殻から構成されており、新皮質や他の脳領域からの広範な投射を受け取ります。

[[Rubicon]] フレームワークによると、脳内のこの中心的な位置と接続性は、基本的な運動制御から目標主導の計画に至るまで、あらゆるレベルでの行動の制御において BG が果たす中心的な役割と一致しています。人形遣いからオーケストラの指揮者に至るまで、その役割をさまざまな比喩を使って説明します。この分野での中心的な疑問の 1 つは、BG が一種の勝者総取り (WTA) プロセスとして動作を選択する責任があるのか​​ (その結果、単一の個別の動作が選択される)、それとも下流の運動経路のより段階的で連続的な並列調整を提供し、最終的な動作の選択はこれらの下流領域で行われるのかということです。後者の役割 (つまり、_遅い選択_) は、強力な計算上の制約と、同時にさまざまな方向に引っ張られる多くの文字列の人形マスターのアナロジーと一致する、かなりの現存データによってサポートされています。

{id="figure_bg-anatomy" style="高さ:25em"}
![大脳基底核 (BG) の主要領域と関連する脳領域。線条体はBGへの入力層であり、大規模な解剖学的特徴として尾状核と被殻で構成され、細長い尾状核は地形的に組織化された感覚運動入力を受け取ります。線条体は脳全体、特に新皮質から広範囲の入力を受け取ります。右のパネルは、前方が左になるように反映された、線条体の 3D ボリュームに対するこの接続性の機能的解釈を示しています (Pauli et al., 2016)。腹側線条体 (側坐核 NAc を含む) は、目標駆動型の感情回路 (「価値」) の入力領域であり、隣接するより背側の領域が運動関連の価値信号をコード化しています。被殻は、げっ歯類の背外側線条体に相当する、運動野および関連する体性感覚皮質と主に相互接続されています。外淡蒼球 (**GPe**) は処理の次の段階で、BG の中心「コア」統合を提供します。GP 内 (**GPi**) は BG 出力を皮質下に送信し、**視床**への広範な接続を介して前頭新皮質にバックアップします。黒質網状部 (**SNr**) も主要な出力経路であり、緻密部 (SNc) は BG 全体にドーパミン神経支配を提供します。視床下核 (**STN**) も前頭領域から広範な皮質入力を受け取り、GPe と双方向に相互作用します。主要な感情/感情領域である **扁桃体** は、BG の一部ではありませんが、BG と広範囲に相互作用しており、尾状尾の最端に位置しており、外側手綱核 (**LHb**) は、他の多くの領域の中でも特に腹側線条体からの広範な入力に基づいて、ドーパミンの低下を促進する上で重要な役割を果たしています。](media/fig_bg_anatomy_human.png)

{id="figure_bg-rat-nhp" style="高さ:25em"}
![ラットとNHP（ヒト以外の霊長類、つまりマカクザル）の線条体の右半球を通る冠状スライス。主要な前頭部からの投影を示し、前の図と一致するルビコンフレームワークに基づく機能的解釈が示されています。最も腹側および内側の領域は、ラットの辺縁下 (**IL**) 前頭皮質から最も強く受信します。これは霊長類のブロードマン野 25 と相同です。これは、ルビコンの目標結果のより抽象的な (純粋な) 値をエンコードします。そこから外側に進むと、より外側の OFC (眼窩前頭) 領域 (**VOLO** = 腹外側 OFC) があり、目標結果のより具体的な感覚的側面をコード化します。そこからの背側には、背側ACC（前帯状皮質、ブロードマン24、**Cg** = ラットの帯状部）から受け取る領域があり、さまざまな行動に関連する感情値（主に努力、不確実性などのコスト）をコード化しています。その内側に移動すると、ラットの前辺縁 (**PL**) 皮質 (Brodmann 32、霊長類の前性 **ACC**) から受信し、IL と ACC の入力を統合して全体の効用 (結果の価値からコストを引いたもの) を計算します。 PL/32 は、内側領域と腹側領域のほとんどにわたってプロジェクトを実行します。ハイルブロナーら、2016 年より。](media/fig_bg_areas_heilbronner_etal_2016_labels.png)
...
以下のセクションでは、背外側 BG によってサポートされる運動制御機能に主に焦点を当てて、BG の反対側の経路組織を検討します。次に、最近発見された新しい接続パターンを組み込んだ BG の新しいモデルについて説明します。この _PCore_ モデルは、淡蒼球外球 (GPe) 内の 2 つの異なるタイプのニューロンと、GPe から線条体への投影が、意思決定プロセス中に経時的に情報を統合する能力をどのようにサポートしているかを示しています。これは、BG 関数の純粋なフィードフォワードの古典的なモデルでは不可能でした。次に、PCore モデルのコンテキストで、BG が何を行うかについての一貫した全体的な理解を提供するさまざまなデータを確認します。

最後に、BG 機能の全体像が整ったところで、学習の形成に重要な役割を果たす、背側線条体に [[#compartmental organization]] を持つ BG 回路の追加要素に目を向けます。これらには、線条体の 85% を構成し、以下の議論の主な焦点である _matrix_ ニューロンの運動および皮質出力の代わりにドーパミン領域に投射する [[#striosomes]] と、数は少ないが残りの線条体ニューロンに対する神経調節的影響が大きい [[#CINs]] (コリン作動性介在ニューロン) が含まれます。

## 相手の経路: D1 vs D2 / Go vs No

{id="figure_bg-rat" style="高さ:25em"}
![齧歯動物の脳の大脳基底核。2 つの主要な経路 (マトリックス / マトリソーム内のニューロン内) を示しています。線条体から **GPi** (齧歯類の淡蒼球球、齧歯動物の脚内核 (_EP_) としても知られる) と **SNr** (GPi よりも大きく、運動出力がより多い黒質網様体) の 2 つの出力領域への直接経路です。経路)、および GPe (淡蒼球外球) を介して追加のホップを行う間接的です。 BG はラット前脳の大きな部分を表すことに注意してください。図はGerfen & Surmeier、2011年より。](media/fig_bg_anatomy_rat.png)

[[#figure_bg-rat]] は、齧歯動物の脳内の BG の解剖学的構造を示しており、線条体のニューロンのマトリックス集団から網状黒質 (SNr) および内淡蒼球 (GPi) (齧歯動物の内部足核 (EP) 核としても知られる) の出力領域までの 2 つの異なる経路を強調しています。これら 2 つの経路は、_direct_ (線条体から SNr/GPi 出力に直接進む) と、出力に進む前に GPe で別の停止を行う _indirect_ です。げっ歯類では、SNr はかなり大きく、運動野と相互接続されていますが、GPi には、後述するように [[#striosomes]] によって駆動される [[lateral habenula]] への投影もあります。ここでは、GPi の SNr 部分とモーター部分を機能的に同一のものとして扱います。また、後述するように、システムの腹側/内側部分に特有の [[#ventral pallidum]] を介した広範な出力経路もあります。

{id="figure_bg-gating" style="高さ:20em"}
![BG の直接および間接経路機能の古典的モデル。直接経路は BG 出力核 (SNr/GPi) を阻害し、これにより視床などの下流標的を脱阻害します (つまり、正味 _Go_ 許容効果)。間接経路は、持続的に活性な GPe ニューロンにもう 1 つのステップを追加します。そのため、間接経路が活性化されると、この GPe 阻害が阻害され、出力核がより活性になり、下流の標的に対する阻害が増加または維持されます (つまり、正味の効果は「なし」または阻害効果)。経路の各ステップに隣接するプロットは、Go の場合の経時的なニューロンの活動を示しています。直接経路のニューロンは強く活性化され、間接経路はそれほど活性化されていません。ドーパミンは、D1 (Go) 対 D2 (No) 受容体を介して直接および間接的な経路の一貫した学習と調節を促進します。そのため、ドーパミンのバーストは Go 経路を促進し、No を阻害しますが、ドーパミンの浸漬ではその逆が当てはまります。](media/fig_bg_gating_circuit.png)

BG の主要なニューロンはすべて抑制性であるため (主要なニューロンが興奮性である皮質とは異なり)、直接経路と間接経路は最終的に反対の効果をもたらすことになり、この事実が BG の機能に関する理論化の多くを推進しています。線条体の直接経路 SPN ニューロン (_dSPN_) は、SNr/GPi 出力を直接抑制します。SNr/GPi 出力は、そうでなければ強直に活動するため、出力ターゲットを継続的に抑制します。したがって、dSPN ダイレクト パスウェイは出力ターゲット上で net_disinhibitory_ となり、「Go」パスウェイとして機能します。つまり、青信号 ([[#figure_bg-gating]]) のように動作を続行できるようになります。このアナロジーは、脱抑制の寛容 (_modulatory_) 性質も捉えています。脱抑制は、それ自体で何かを強制するのではなく、存在する可能性のある「トラフィック」の進行を許可するだけです。
...
![アクション選択を実行する BG の概念モデル。選択された 1 つのアクションのみ (直接 Go 経路経由) の進行を許可し、他のアクションは間接的な No 経路経由で抑制されます (Gazzaniga et al、2018 より)。](media/fig_bg_action_sel_dam.png)

アクション選択モデルの最も単純な形式は、[[#figure_bg-act-sel]] に示すように、選択されたアクションが BG から _Go_ 抑制解除シグナルを受け取る一方、他のすべての選択されていないアクションは _No_ 抑制シグナルを受け取ると仮定します。
...
![マウスの BG 回路を介した並列ループにより、BG と相互接続されたモーターおよびその他の脳領域の制御を学習するための高度な並列検索プロセスがサポートされます。 SNr (ニグラ) 出力は、おそらく特定の運動効果器 (筋肉群) 周辺の関連する線条体信号を統合しており、そのデータは直接および間接経路から同じ SNr ニューロンへの強力な収束を示しています。 SNrの解剖学的構成は線条体に対して反転しており、SNrの腹側領域が背側線条体の入力を処理していることに注意してください。筋膜傍核 (PF、層内視床の一部) は、PCore モデルの線条体に重要なクレジット割り当てフィードバック信号を提供します。フォスターら、2021 年より。](media/fig_bg_loops_foster_etal_21.png)
...
![大脳基底核の PCore モデル。BG 回路の他のすべての部分に影響を与える GPe ニューロンの内外への複数の投射を中心としており、GPe 淡蒼球をその機能の中核に据えています。 **GPeAk** (arkypallidal) ニューロンは直接経路の線条体ニューロン (**dSPN**) から情報を受け取りますが、原型 (**GPePr**) ニューロンは古典的モデルと同様に間接経路から情報を受け取ります (そのため名前が付けられています)。 GPeAk は線条体まで抑制を逆行させるため、SPN ニューロンを脱抑制するには GPeAk を抑制する必要があり、これは直接経路入力によって達成されます。 **iSPN** ニューロンは、活性が高く、dSPN を直接阻害している場合には、GPeAk を阻害することである程度の症状を軽減することもあります (ただし、その逆はありません)。 **STN** へのハイパーダイレクト パスは、システムに初期の「ブレーキ」をかけて、時期尚早な応答を防ぎます。各核の下の数字は、ラットの各半球のニューロンの大まかな数を示します (Oorschot、1996)。](media/fig_pcore_v2_pr_ak.png)

この新しい回路の実装は、[[#figure_pcore-bio]] に要約されており、GPe ニューロンの 2 つの主要なサブタイプを示しています。_GPeAk_ は、分子マーカー NPAS1 および FOXP2 を発現する _arkypallidal_ GPe ニューロンであり、GPePr は、古典的モデルの GPe と同様の接続パターンを持ち、PV を発現する _prototypical_ GPe ニューロンです。 (パルブアルブミン、[[inhibition]] で説明されている) および KCNG4。図に示されているように、GPe ニューロンの約 45% は原型的であり、18% は有核淡蒼球で、さらに 12% が SNc ドーパミン領域に投射しています (線条体の [[#striosomes]] ニューロンと同様、これについては以下で説明します)。残りのニューロンはより不均一なグループを構成しますが、当面は無視します。
...
![Go > No ケース (左パネル) vs No > Go ケース (右パネル) の PCore モデルのダイナミクス。GPeAk ニューロンの中心的な役割を示しています。各層には、層ごとに 25 個のニューロンにわたるスパイクのラスター プロットが表示され、各層の深さは時間を遡って表示されます。したがって、ハイパーダイレクト経路入力によって駆動される STN の活動の初期バーストがわかります。これにより、開始時に「ブレーキ」がかかり、STN を抑制する GPePr によってブレーキが解除されると、残りのダイナミクスが展開できるようになります。強調表示されている各手順の詳細については、本文を参照してください。線条体ニューロンは、dSPN = Go および iSPN = No で、マトリックス (対ストリオソーム) サブセットを表す Mtx としてラベル付けされています。](media/fig_pcore_v2_vs_sim_go_no.png)

[[#figure_pcore-dyn]] は、最終的に正味脱抑制 (「Go」) および正味抑制 (「No」) になる場合の動作中の PCore モデルを示します ([[BG ventral simulation]] を参照)。 Go の場合、主な手順は次のとおりです。

0. STN ハイパーダイレクト アクティビティは、SNr/GPi、GPePr、および GPeAk ニューロンにバースト的な活性化を提供し、SNr/GPi 出力の抑制を効果的に防ぎます。これはシステムの最初の「ブレーキ」であり、GPePr が STN を阻害すると解除され、[[neuron channels#SKCa]] カルシウム依存性 K チャネルがより長時間持続する抑制休止状態を生成し、BG が出力経路を制御するためのウィンドウ (すなわち、ゲート ウィンドウ) を提供します。

1. 「ACC」入力 (このモデルでは単にクランプされた入力層) から dSPN へのより強力な学習重みにより、これらのニューロンは相手の iSPN ニューロンよりも激しく反応します。

2. この dSPN 活性は GPeAk を直接阻害し、全体的な「Go」モード活性をさらに促進する「正の」脱抑制フィードバック ループを開始します。これは事実上一種の増幅回路であり、広範囲の異なる入力強度にわたって Go と No を区別できるようにするために重要です。

3. GPeAk 活性の低下により線条体ニューロン (dSPN と iSPN の両方) が脱抑制され、これにより dSPN 経路がさらに増加し​​ますが、これは dSPN 細胞を直接阻害する iSPN 経路の発火増加によっても抑制されます。

4. 線条体 dSPN 活性の増加により、SNr/GPi ニューロンが抑制されます...

5. それによって、BG の下流の標的、この場合は視床を脱抑制するという古典的な Go 経路の力学が達成されます。

6. この段階的出力脱抑制は、STN ニューロンが活動休止から回復すると終了し、GPeAk が興奮し、SPN が抑制されます。 SNr/GPi 出力核でもブレーキが再びオンになります。

右側に示されているケースは、iSPN _No_ 経路が入力によってさらに励起されたときに何が起こるかを示しています。

1. これで、iSPN ニューロンが最初はよりアクティブになります。

2. GPePr を阻害し、それによって GPeAk を脱阻害します。

3. したがって、GPeAk 活性が継続すると線条体が阻害され、dSPN Go 経路の活性化が妨げられます。

4. したがって、SNr/GPi はアクティブなままになります...

5. そして視床は SNr/GPi によって抑制されたままになります。
...
![iSPN (a) または STN (b) ニューロンの光遺伝学的選択的活性化の結果。 ISPN を活性化すると GPePr (「Proto」) ニューロンが抑制される一方、GPeAk (「Arky」) ニューロンは脱抑制されました。 STN を活性化すると、GPeAk ニューロンよりも GPePr ニューロンがより強く活性化され、GPePr へのより強い投影と一致します。](media/fig_ketzef_etal_21_ispn_stn_stim.png)
...
![前頭葉皮質から下行運動野までのBGに関与する回路は、ラットの解剖学的構造に従って概略的に配置されています（以前の図2および3と同様）。腹側回路と内側回路は図 2 に示すとおりです (V = 腹側、M = 内側、D = 背側、L = 外側、S = 線条体、aIC = 前島)。一次運動野と二次運動野は背側線条体と外側線条体を標的とし、前方運動野（霊長類のはるかに大きなdlPFCの弱い類似体）は背側内側線条体を標的とします。 SNr 出力には、図に示すような地形図も含まれています。 DLS 出力の対象となる視床核は腹側内側と腹側外側であり、これらも小脳と相互接続しています。脳幹の運動ターゲットには、上丘 (SC; 方向付けとアプローチ)、髄様網様体 (MRF; 移動)、および橋網様体 (PRF; 四肢、指) が含まれます。](media/fig_motor_systems.png)
...
![ラットの腹内側線条体 (VMS) のニューロンの活動。異なる手がかりモダリティでの獲得および再獲得の全過程にわたって、単純な手がかり T 迷路課題で記録されました (AA1 = 初期聴覚手がかり獲得、AA2 および AO はさらなるトレーニングを示し、その後 TA = 触覚手がかり)。 VMS ニューロンは、開始時と終了時に一貫して発火しますが、ラットが合図を感知して右腕または左腕に進入することを決定する重要な決定点では発火しません。これは、最初に包括的な目標/計画を設定し、最後に実際の結果で更新することと一致しています。上の図はニューロンの母集団に対する Z スコア付けされたデータを示し、下のパネルは個々のトライアルの実際の発火率とラスター プロットを示し、クリック応答ニューロンと報酬応答ニューロンの個々のニューラル スパイクを記録します。活動は一般に非常にまばらであり、それぞれ警告の合図と報酬の開始によって明らかに強く駆動されます。図は Atallah et al.、2014 から引用。](media/fig_bg_vs_gating_atallah_etal_2014.png)
...
![自由に行動するマウスの DLS における dSPN と iSPN の神経活動 (黄色) と不活動 (青色) の痕跡の平均。標識されたさまざまな動作に関連付けられます。各線は、個々のニューロンの動作的に同期された平均を表します。これらの動作の開始にまたがって、各動作に特有の両方の経路で神経活動の双方向の調節が存在します。任意のニューロンには比較的短い活動の位相ウィンドウがあり、これは動作のインスタンス全体でほぼ同じ相対的な時点で確実に発生します。マルコウィッツら、2018 年より。](media/fig_bg_dorsal_activity_markowitz_etal_18.png)
...
![SNr BG のニューロンからの記録は、マウスの X、Y 頭の位置と強く相関する核を出力します。神経発火率の観点から、これらの連続的な頭部位置変数が段階的にエンコードされていることに注目してください。他のニューロンは、他の連続運動変数 (速度など) と相関しており、(示されているように) 正相関だけでなく逆相関のケースも存在しました。 Barter 他、2015 年より。](media/fig_bg_dorsal_snr_barter_etal_15.png)
...
![図に示すように、腹側淡蒼球 (**VP**) の接続性は、複数の異なる神経伝達物質を備えた多様な細胞タイプを持ち、すべての主要な動機付け / 感情 / 神経調節脳領域に出力を送信します。 NAcc = 側坐核（腹内側線条体）。 **BLA** = 扁桃体基底外側。 **LH** = 視床下部外側部。 **VTA** = 腹側被蓋野 (ドーパミン)。 **MD** = 視床内側背筋。その後、腹側および内側前頭前野 (v/mPFC) に広範囲に投影されます。 **LHb** = 外側手綱核; **DR** = 背側縫線（セロトニン）。 Root et al.、2015 の図。](media/fig_vp_conns_root_etal_15.png)
...
このトポグラフィーには、共通のモーター出力信号に寄与する個々の SPN ニューロン間の学習を調整する追加のタイプのニューロンが重ねられています。これらのニューロンは、最終的に行動行動から生じる相性 [[dopamine]] _critic_ (報酬予測誤差) 信号に関連して、この運動経路の実際の寄与と活動に特化したクレジット割り当て信号を提供します。この特殊なクレジット割り当ては、より正確なクレジットと責任を割り当てるために不可欠であり、したがって、純粋にグローバルで未分化の批評家信号のみが個々の活動に比例してすべてのニューロンに適用される代替方法と比較して、並列検索プロセスが大幅に高速化されます。

{id="figure_compartments" style="高さ:30em"}
![背側BGの区画構成。ストリオソームパッチと周囲のマトリックス、および通常パッチの境界に見られるコリン作動性介在ニューロン（ChI、別名CIN）を示しています。ドーパミン軸索はこれらの領域を広く神経支配します。 Brimblecombe & Cragg の図、2017 年。](media/fig_bg_compartments_dls_brimblecombe_cragg_17.png)

[[#figure_compartments]] には、次の要素が含まれます。

* **ストリオソーム**のパッチ**。背側線条体 ([[@GraybielRagsdale78]]; [[@Gerfen92]]) に集まっており、これまでの議論の焦点である **マトリックス** ニューロンに囲まれています。これらのパッチ ニューロンは、SNc 内のドーパミン ニューロンの発火を双方向に制御する明確な投射を備えており、その後、これらの同じ解剖学的領域に戻って投射します ([[@Stephenson-JonesKardamakisRobertsonEtAl13]])。これらのパッチ ニューロンの接続性は、上で論じた腹側淡蒼球を介した腹内側線条体の接続性と類似しており、局所グループの活動に基づいて特殊なドーパミン信号を提供する局所的批評システムの要件に顕著に適合します。
...
![ストリオソームからの直接および間接経路出力とマトリックスからの出力。直接経路ニューロンはSNcのドーパミンニューロンを抑制するが、間接経路ニューロンは淡蒼球の領域に投射し、外側手綱核（LHb）への興奮性投射があり、ドーパミン発火の低下を引き起こすことができる。 Grillner et al.、2020 の図。](media/fig_bg_striosomes_grillner_etal_20.png)

{id="figure_gph" style="高さ:35em"}
![淡蒼球手綱核 (GPh) 経路には、ストリオソームから、GABA に加えてグルタミン酸を発現する GPi /足足内核 (EP) ニューロンのサブセットへの投射が含まれ、外側手綱核に投射します。これは、ストリオソームがドーパミンの発火を調節できる重要な経路です。 Stephenson-Jones et al.、2013 の図。](media/fig_bg_gph_stephenson-jones_etal_13.png)
...
![BG 背側配列学習モデルにおける正しいアクション プールと間違った (エラー) アクション プールのパッチ ニューロンにおける直接 (D1) 経路活動。これらのパッチ ニューロンは、平均して正しいアクションと正しくないアクションを区別することを比較的早く学習し (正解 > エラー)、プール固有の有用な追加の批評家調整を提供します)。](media/fig_bgdorsal_patch_dad1_epoch.png)
...
結果の時点で計算される重みの変化は、ドーパミン変調 _DA_ (バーストの場合は正、ディップの場合は負) に蓄積されたトレース値を乗算したものになります。

{id="eq_vms-dwt" title="VMS 重量の変更"}
$$
...
{id="eq_dls-tr-pf" title="DLS PF / CIN がアクティブです"}
$$
...
{id="eq_dls-tr-off" title="DLS PF / CIN オフ"}
$$
...
![単一シーケンスの試行にわたる、BG 背側シーケンス学習モデルのアクティブ プールのパッチ ニューロンにおける直接 (D1) 経路活性と間接 (D2) 経路活性。システムはシーケンス内の最初と最後のアクションを間違っており、これは図に示すように学習されたパッチ値に反映されています (D1 は正しい試行には強く、不正確な試行には弱くなりますが、D2 にはその逆が当てはまります)。アクティブ ラーニング ルールは、D1 アクティビティが強力であるため、正しいアクションに関する学習を割引きする一方で、間違った最初と最後の試行についてはより多くの学習を推進します。](media/fig_bgdorsal_patch_da_trial.png)
...
<!--- Notes from Tom: clarify heterogeneity of VS -- lots of evolutionary stuff in there in addition to key goal projecting neurons in the MD.  GPePr -> GPePr 阻害は拡散性ですが、GPeAk -> 線条体はより局所的です。 * 実際に da を解放できるパッチ ニューロンに戻って、最後の da からトレーニングを受けてみます。これはクレジット割り当てモデルというよりは連鎖モデルです。 DMS は DLS に似ていますが、DLPFC 用 -->
