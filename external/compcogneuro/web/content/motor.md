---
layout: default
title: "compcogneuro/web: motor"
description: "compcogneuro/web: motor.md の日本語訳"
article_type: 翻訳資料
...
<p>原典: <a href="https://github.com/compcogneuro/web/blob/main/content/motor.md" target="_blank" rel="noopener noreferrer">https://github.com/compcogneuro/web/blob/main/content/motor.md</a></p>
...
bibfile = "ccnlab.json"
+++
脳の **運動** システムには多くの異なる領域が関与しており、さまざまな方法で脳全体が運動出力を生成するために存在しています ([[#figure_overview]])。たとえば、ダニエル・ウォルパートは、ホヤは動き回る必要がなくなったら自分の脳を食べると主張しました。モーター制御のさまざまな側面のより具体的な説明は、[[reinforcement learning]]、[[basal ganglia]]、[[cerebellum]]、[[space]]、および [[Rubicon]] フレームワークにあります。このページでは、運動制御の課題と解決策の高レベルの概要を提供し、筋肉の基礎的な生理機能と、脊髄と脳幹が運動制御の体系的な基盤をどのように提供するかについての詳細を示します。

{id="figure_overview" style="高さ:35em"}
![ヤツメウナギ (進化上最も古い脊椎動物の 1 つ) と霊長類の運動系の解剖学的組織。脊髄内の介在ニューロン間のCPG（中枢パターン発生器）回路は、脳幹、小脳、大脳基底核、および大脳皮質のより高いレベルが下行経路を介して制御する、低次元の適応性のある（局所固有受容および体性感覚フィードバックを介した）_筋肉相乗効果_のレパートリーを提供します。皮質ネコ科動物やその他の種を対象とした広範な研究で示されているように、「ヤツメウナギの核」システムは生存に十分であることが示されています。グリルナー＆エルマニラより、2020年。](media/fig_motor_primate_lamprey_grillner_20.png)
...
これは基本的な [[search]] 問題のもう 1 つの例であり、運動システムの場合、[[evolution]] は脊髄と脳幹に複雑な **筋肉の相乗効果**を組み込むことで多くの仕事を行い、**介在ニューロン** (興奮性および抑制性) のネットワークが筋肉活性化の **時空間パターン**を活性化します。この低次元の要素 (つまり [[linear algebra#basis space]]) から、運動動作の複雑なパターンが構築されます ([[@Bernstein67]]; [[@Bernstein96]]; [[@TreschSaltielBizzi99]]; [[@dAvellaSaltielBizzi03]]; [[@TingMcKay07]]; [[@BizziCheungdAvellaEtAl08]]; [[@TreschJarc09]]; [[@OverduindAvellaRohEtAl15]]; [[@Latash20]];
...
文献では、これらの相乗効果を指すために、**反射** ([[@Sherrington10]])、**中枢パターン生成装置** (CPG、[[@GrillnerElManira20]]; [[@GrillnerZangger79]])、および_力場_ ([[@GiszterMussa-IvaldiBizzi93]]) など、さまざまな異なる用語が使用されています。特に CPG の場合、これらの用語には多少異なる意味合いがあり、これは、たとえば長期間にわたる持続的でリズミカルなパターン化された行動を含む、運動行動を駆動する複雑な回路に当てはまります。より一般的には、これらの相乗効果は、_モジュール_ ([[@BizziCheungdAvellaEtAl08]])、_プリミティブ_、ビルディング ブロック、または運動動作のコンポーネントと呼ばれます。

{id="figure_synergies" style="高さ:30em"}
![カエルの蹴り動作から抽出された3つの筋肉のシナジー。 a) 時間経過 (水平軸) に伴うさまざまな筋肉の活性化 (垂直軸)、与えられた全体的な運動動作の達成 (相乗効果の対応する列の順序で並べられた腕のモーメント): _HE_ 股関節伸展。 _HF_ 股関節屈曲。 _KE_ 膝の伸展。 _KF_ 膝の屈曲。 _AE_ 足首の伸展。 _AF_ 足首の屈曲。 b) 6 つの異なる全体的な運動動作を再構築するために必要な、各相乗効果コンポーネントの経時的な活性化強度 ($C_{1-3}$)。これらのパターンは主に時間的パターンではなく大きさが異なり、実際、時間的パターンは相乗効果自体のパターンと一致しているように見え、より静的な制御信号がほぼ同じ結果を達成できることを示唆しています。 d'Avella 他、2003 年より](media/fig_muscle_synergies_davella_etal_03.png)
...
![中脳から延髄までの複数のレベルにわたる、脊髄筋の相乗効果に対する脳幹レベルの制御。 MLR = 中脳運動領域。 SC = 上丘; CnF = 楔状核; PPN = 脚橋核; mRT = 中脳網様体形成。 LPGi = 側傍巨細胞核。 IC = 下丘; 5N = 三叉神経運動核。 Gi = 巨大細胞核。 7N = 顔面運動核。アーバー＆コスタ、2022年よ​​り。](media/fig_motor_brainstem_arber_costa_22.png)
...
![筋肉は反対側のペアで構成されており、_伸筋_の収縮により四肢 (上腕三頭筋など) が伸長され、一方、屈筋_の収縮により四肢の屈曲 (圧縮) が引き起こされます。](media/fig_muscle_extension_flexion.png)

骨格筋は [[opponent]] ペア ([[#figure_muscle]]) に編成されており、**伸筋**の収縮により四肢が外側に拡張され (_extension_)、**屈筋**の収縮により四肢が内側に収縮されます (_flexion_)。筋肉は、**腱**を介して骨に接続されています。腱は、コラーゲンでできた強力でわずかに弾性のあるロープ状の組織です。腱は通常、関節の周囲を包み込み、より大きなてこの作用を得るために骨のさらに下で接続することによって力を伝達します。
...
![眼球運動制御システム。 a) _直筋_は目の動きに関与しており、水平方向の動きは内側直筋 (mr) と外側直筋 (lr) のバランスによって制御され、垂直方向の動きは上直筋 (sr) と下直筋 (ir) によって制御されます。 b) これらの筋肉は、外転核 (ABN) 運動ニューロンの制御下にあり、特定のニューロンのスパイク ラスター プロットの上の H = 目の水平位置のプロットに示されているように、神経発火率と目の位置および速度の間に線形関係があります。上のパネルでは、ニューロンが高周波バーストを発火して右方向の眼球運動 (プロットでは右 = 上) を駆動します。これはニューロンの位置 (右半球、右向きの目の筋肉を収縮) と同側です。その後の強直性発火率はその後の目の位置に直線的に関係し、目をこの中心から外れた位置に保つのに必要な収縮を維持します。下のパネルは左方向の動きを示しており、ニューロンは動き中に一時停止し、その後、より低い緊張性発火率を確立します。 c) 2 つの異なるニューロンの目の位置と発火速度の間の線形関係。同側と対側の発火（相手のプロセスとして）の違いにより、目のパラメータの最も正確な予測が得られます。スパークスより、2002 年。](media/fig_bg_motor_eye_abducens_sparks_02.png)
...
![眼球運動脳幹ネットワーク。 a) サルの矢状面における領域の解剖学的構成: MRF = 中脳網様体。 III = 眼球運動核。 IV = 滑車核; VI = 外転核; Med RF = 髄様網様体形成 (サッカード生成ニューロン)。 NPH = 舌下前置核（前庭核）。 PPRF = 傍正中橋網様体 (水平サッカードを駆動する)。 rlMLF = 内側縦束（垂直サッカード）の吻側間質核。 SC = 上丘 (視蓋)。 b) 水平バーストを生成するネットワーク (BN = バースト ニューロン、PPRF および Med RF に位置): EBN = 外転ニューロンの速度バースト コーディングを直接駆動する興奮性バースト ニューロン。 IBN = 抑制性。 LLBN = 長いリード (結合強度が低い)。 MVN = 内側前庭核。 OPN = オムニポーズニューロン。サッカードは、ＯＰＮを抑制するトリガー（Ｔｒ）入力によって開始され、その後、サッカード中にＥＢＮによって興奮するラッチ（Ｌａ）ニューロンによって抑制される。スパークスより、2002 年。](media/fig_bg_motor_eye_brainstem_sparks_02.png)
...
筋肉のシミュレーションに使用される標準方程式は、もともと [[@^Hill38]] によって開発された単純化された近似に基づいており、したがってヒル型筋肉として知られています ([[@RitchieWilkie58]]; [[@Zajac89]]; [[@MillardUchidaSethEtAl13]]; Millard (2013) バージョンの実装については OpenSim を参照してください: [[@SethHicksUchidaEtAl18]]; [ギットハブ](https://github.com/opensim-org/opensim-core/blob/main/OpenSim/Actuators/Millard2012EquilibriumMuscle.cpp))。この筋肉モデルには、並行して動作する 2 つの要素 ([[#figure_hill-muscle]]) があります。1 つは収縮する実際の筋肉を表す _contractile_ 要素 (CE)、もう 1 つは CE と平行に実行され、筋肉の軟部組織のバネのような特性を表す _passive_ 要素 (PE) です。腱は、筋肉 CE と直列 (SE) のバネ状要素を介して表されます。
...
{id="eq_hill-about" title="ダンプ ヒル タイプ モデル"}
$$
v = \frac{f_T(l_T) sec(\alpha) - f_{PE}(l)}{a f_L(l) f_V + \beta}
$$
ここで、$f_V$ は、$v$ の指定された値に関連する傾きを表します ($v > 0$ かどうかに応じて、それに収束するには 2 回の反復が必要です)。

筋肉をシミュレートするさらに簡単で精度は低い方法は、[コージェントコアラボの物理学](https://www.cogentcore.org/lab/physics) モデルで行うように、標準の物理シミュレータ ジョイントの剛性と減衰パラメータを操作しながら、ジョイントの位置の値を更新することです。これには、(平衡点制御の場合と同様に) 目標の関節位置を知る必要がありますが、剛性パラメーターは関節の動きの速度に比例し、減衰によって減衰されたバネのダイナミクスが提供されます。
...
![脊椎介在ニューロンのカテゴリを特定し、背側と腹側の位置に従って整理し、生年月日順に並べました。ダブルクリックすると大きなバージョンが表示されます。ビコフより、2019年。](media/fig_spinal_cord_interneurons.png)
...
**前角**灰白質には筋肉の収縮を直接引き起こす一次運動ニューロンが含まれており、**中間**灰白質領域（脊髄の特定のレベル）には感覚ニューロンと運動副ニューロンが含まれています。
...
![脊椎猫の後肢運動の多くの側面を正確にシミュレートできる脊髄中枢パターン発生器 (CPG) 回路の詳細な回路ベースのモデル。介在ニューロンは、伸筋を表す -E と屈筋を表す -F という接尾辞を付けて機能別にラベル付けされています。RG = リズム生成。 PF = パターン形成。 IN = 抑制性介在ニューロン。オレンジ色の_速度、長さ_ニューロンは、運動ニューロン(屈筋に限定)に重要な感覚フィードバックを提供する固有受容(紡錘線維)ニューロンであり、力を生成する運動ニューロンからの重要な遠心性コピーフィードバックもあります。下降する脊髄上ドライブ入力は、運動ニューロンではなく、最上位の RG 介在ニューロンに入力されることに注意してください。篠原ら、2025 年より。](media/fig_spinal_cord_cpg_shinohara_etal_25.png)
...
![小脳が脊髄筋の相乗効果（CPG：中央パターン発生器）の調整を制御できる経路。 DSCT = 背側脊髄小脳路。 VSCT = 腹側脊髄小脳路。 MLR = 中脳運動領域。 LGCI = 側傍巨細胞核。グリルナー＆エルマニラより、2020年。](media/fig_cerebellum_cpg_tuning_grillner_20.png)
...
![SNr (網状黒質) を介した大脳基底核のモーター出力経路。これらの広範な出力により、大脳基底核は、強力なドーパミンベースの強化学習メカニズムを使用して、これらの領域の筋相乗プログラムの適切なシーケンスを構築することができます。アーバー＆コスタ、2022年よ​​り。](media/fig_bg_motor_outputs_arber_costa_22.png)
...
![一次感覚 (S1) 皮質領域と一次運動 (M1) 皮質領域の「ホムンクルス」 (「小さな人」) 組織。これらは互いに中心溝を挟んで位置し、密に相互接続されています。 S1 は、皮質皮質経路を介して直接的に、および視床後内側核 (POm) を通過する突起を介して間接的に、M1 に強力なドライバー入力を提供します。](media/fig_sensory_motor_homunculus.jpg)
...
![運動処理の「ビッグループ」。これは、前頭皮質を訓練するための一種の予測学習をサポートする可能性がある。視床（VL = 特に腹外側部）は、DCN（深部小脳核）からの強力なドライバー入力と、運動皮質からの標準的な非ドライバー入力を受け取り、特定の運動トレーニング信号を提供できると思われる焦点（「コアのような」）入力を運動皮質に投射します。これら 2 つの入力間の経時的な差により、時間微分誤差駆動型学習メカニズムが駆動される可能性があります。この小さなループには、有用な特定のトレーニング信号を提供しない可能性が高い広範な (「マトリックス状」) 接続を備えた視床の別個の領域 (VA、VM) が含まれます。むしろ、皮質における準備運動状態から駆動運動状態への移行を促進するために重要である可能性があります。アーバー＆コスタ、2022年よ​​り。](media/fig_motor_big_loop_arber_costa_22.png)
