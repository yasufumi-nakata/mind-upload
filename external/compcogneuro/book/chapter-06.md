---
layout: default
title: "compcogneuro/book: 知覚と注意"
description: "compcogneuro/book: chapter-06.md の日本語訳"
article_type: 翻訳資料
...
<p>原典: <a href="https://github.com/compcogneuro/book/blob/main/chapter-06.md" target="_blank" rel="noopener noreferrer">https://github.com/compcogneuro/book/blob/main/chapter-06.md</a></p>
...
教育学的に、知覚は私たちが認知現象に入る基礎として機能します。これは、認知領域の中で最もよく研​​究されており、生物学的に根拠のある領域です。その結果、私たちは知覚に関する多くの魅力的な現象のうちのほんの一部のみを取り上げ、主に視覚に焦点を当てます。しかし、私たちは他の知覚現象の背後にある一般原理の多くを捉える一連の中核的な問題に焦点を当てています。
...
V1 で学習した機能に基づいて、物体が網膜に投影される際の表面の外観の大きなばらつきに関係なく、**腹側何**経路のより高いレベルがどのようにして**物体を認識**できるかを調査します。物体認識は、階層的に構成された一連の特徴カテゴリ検出器が、非常に困難な全体的な問題をどのように段階的に解決できるかを示す典型的な例です。この原理に基づいた計算モデルは、現実的な視覚画像上で高レベルの物体認識パフォーマンスを示すことができるため、おそらくこれが脳がこの問題を解決する方法である可能性が高いという説得力のある示唆を提供します。

次に、**空間注意**における**背側どこ** (またはどのように) 経路の役割を検討します。空間的注意は、ビュー内に複数のオブジェクトがある場合のオブジェクト認識など、多くのことにとって重要です。空間的注意は、オブジェクトの 1 つに処理を集中させるのに役立ちますが、他のオブジェクトに関連付けられたフィーチャのアクティビティを低下させ、潜在的な混乱を軽減します。処理ストリームの内容と場所の間のこの相互作用に関する私たちの計算モデルは、「どこ」経路への脳損傷の影響を説明することができ、脳の片側のみの損傷に対する半空間無視、および両側損傷を伴うバリント症候群と呼ばれる現象を引き起こします。神経学的に無傷な行動と脳が損傷した行動の両方を説明できるこの機能は、神経ベースのモデルを使用することの強力な利点です。
...
![網膜から視床外側膝状核（LGN）を通って一次視覚野（V1）に至る初期視覚処理の経路。異なる視野（左と右）からの情報がどのように反対側の半球に送られるかを示します。](figures/fig_vis_optic_path.png){#fig:fig-vis-optic-path width=40% }

![網膜が均一な均一照明ではなく、対照的な照明の領域にのみ応答することによって情報を圧縮する方法。網膜細胞の応答特性は、中央領域が狭く周囲が広い (中心周囲受容野とも呼ばれる) ガウス差分 (DoG) フィルターによって要約できます。興奮性成分と抑制性成分は、両方が均一に照明されている場合には正確にキャンセルされますが、周囲よりも中心に光が当たると（またはその逆）、照明が暗い部分と明るい部分の間で切り替わるエッジで示されているように、それらは反応します。](figures/fig_on_off_rfields_edge.png){#fig:fig-on-off-rfields-edge width=50% }

![A V1 simple cell that detects an oriented edge of contrast in the image, by receiving from a line of LGN on-center cells aligned along the edge. LGN 細胞は、受信した興奮と抑制の差によって発火し (前の図を参照)、その後、LGN 細胞から受信した V1 ニューロンを活性化します。](figures/fig_v1_simple_dog_edge.png){#fig:fig-v1-simple-dog-edge width=30% }
...
* 古い学校のプロジェクター刺激を使用した古典的な Hubel & Wiesel V1 受容野マッピング: [YouTubeビデオ](https://www.youtube.com/watch?v=KE952yueVLA)

* 新しい逆相関 V1 受容野マッピング: [YouTubeビデオ](https://www.youtube.com/watch?v=n31XBMSSSpI)

![V1 内の複雑な細胞タイプと単純な細胞タイプ: 複雑な細胞 (図の上半分) は、複数の位置にわたって統合することによってより大きな受容野を作成すること (V1-Simple-Max 細胞はこの空間統合のみを行います)、および極性 (オンとオフのコーディング領域の位置) 全体で抽象化することを含む、単純な細胞のプロパティを統合します。エンドストップ セルは最も複雑で、特定の単純なセルに隣接するあらゆる形式の対照的な方向を検出します。 V1 単純セル (図の下半分) は、[@fig:fig-v1-simple-dog-edge] で説明されているように、コントラストの方向性のあるエッジを検出します。シミュレーターでは、V1 シンプル セルはガボール フィルターを使用してより直接的にエンコードされ、指向性エッジ感度が数学的に記述されます。](figures/fig_v1_visual_filters_hypercols_basic.png){#fig:fig-v1-visual-filters-hypercols-basic width=50% }
...
![一次視覚野 (V1) 以降の視覚処理経路の解剖学的接続性を示す、Felleman & Van Essen (1991) の図の更新および簡略化バージョン。  青の影の領域は腹側の「What」経路を構成し、緑の影の領域は背側の「Where」です。  Markov et al. (2014) から転載。](figures/fig_markov_et_al_cort_hier.png){#fig:fig-markov_cort_hier-6 width=50% }

*ネットワーク*の章で説明したように、一次視覚野を超えて、知覚システムは、階層的に組織された神経検出器層の力の優れた例を提供します。 [@fig:fig-markov_cort_hier-6] は、V1 以降 [@MarkovVezoliChameauEtAl14; @FellemanVanEssen91] までのすべての主要な視覚野の解剖学的接続パターンを示します。厳密な階層の外側にも多くの相互接続があるにもかかわらず、図に示すように、接続の特定のパターンにより階層構造を抽出できます。

![古典的な Ungerlieder と Mishkin (1982) のフレームワークに基づく、視覚処理における *何を* 対 *どこ* (腹側対背側) 経路の分割。](figures/fig_vis_system_bio.png){#fig:fig-vis_system_bio-6 width=50% }

[@fig:fig-vis_system_bio-6] は、これらの領域を解剖学的位置に配置し、視覚処理 [@UngerleiderMishkin82] における **何を対どこ** (**腹側と背側**) の分割をより明確に示します。 V1 から V4 まで腹側方向に**下側頭皮質 (IT)** の領域に向かう投影 (TE、TEO、前の図では後部 IT の PIT とラベル付けされている) は、視覚入力内の物体の同一性 (「何を」) を認識するのに重要ですが、頭頂皮質を通過する投影は、MT および MST 領域の運動信号を含む空間 (「どこ」) 情報を抽出します。この章の後半では、これらの視覚的な処理ストリームがそれぞれどのように独立して機能するのか、また相互に作用して知覚における重要な計算問題を解決するのかを見ていきます。
...
* **IT-anterior** (AIT) --- これは、視覚情報が本質的に非常に抽象的で **意味論的**になる場所です --- それは、さまざまな人、場所、物に関するあらゆる種類の重要な情報をエンコードできます。

対照的に、背側から頭頂皮質（MT、VIP、LIP、MSTの領域）に直接上る視覚処理の**場所**の側面には、動き、深さ、およびその他の空間的特徴の処理に重要な領域が含まれています。

## 一次視覚野の指向性エッジ検出器

![さまざまな方向の棒刺激に応答した個々の V1 ニューロンの方向調整 --- このニューロンは垂直方向の刺激を好みます。](figures/fig_v1_orientation_tuning_data.jpg){#fig:fig-v1-orientation-tuning-data width=50% }

![V1 の指向性エッジ検出器のトポグラフィー構成 --- この色分けされたマップに示されているように、ニューロンの隣接領域は同様の指向性調整を持っています。パネル C に示すように、異なる色が指向性の優先性を示しています。パネル B は、指向性の完全な 360 度ループがどのように中心点の周りで核形成されるかを示しています --- これらは風車構造として知られています。](figures/fig_v1_orientation_cols_data.jpg){#fig:fig-v1-orientation-cols-data width=30% }

一次視覚野（V1）のニューロンは、その受容野（RF --- 特定のニューロンが入力を受け取る視野の領域）内の光のエッジまたはバーの方向を検出します。 [@fig:fig-v1-orientation-tuning-data] は、配向バーに応答した個々の V1 ニューロンの電気生理学的記録からの特徴的なデータを示します。このニューロンは垂直方向に最大限に反応し、その両側で段階的に減衰します。これは、**チューニング カーブ**の非常に典型的な形式です。 [@fig:fig-v1-orientation-cols-data] は、これらの方向調整されたニューロンが*地形学的**に組織化されており、隣接するニューロンが同様の方向をコード化する傾向があり、方向調整が皮質の表面にわたってかなり連続的に変化することを示しています。
...
![自然画像 (山、木など) の小さなウィンドウにさらされた V1 ニューロンのシミュレーションにおける、指向性エッジ検出器の地形的構成。ニューロンの近傍接続により、地形的組織が発達します。](figures/fig_v1rf_map.png){#fig:fig-v1rf-map width=50% }

[CCNシムズ](https://compcogneuro.org/simulations) の `v1rf` を開いて、V1 での指向性エッジ検出器の開発を調べます。このモデルは一連の自然画像にさらされ、方向性のあるエッジをエンコードすることを学習します。これは、これらの画像に存在する統計的規則性であるためです。 [@fig:fig-v1rf-map] は、展開される方向の結果マップを示します。
...
![物体認識が難しい理由: 同じものとして分類されるべきもの (つまり、同じ出力ラベルを持つもの) は、異なる位置やサイズなどに表示される場合、網膜入力特徴に重複がないことがよくありますが、異なるものとして分類されるべきものは、同じ位置に表示される場合に高度な重複が存在することがよくあります。したがって、ボトムアップの類似性構造は、目的の出力類似性構造と正反対であり、問​​題を非常に困難にしています。](figures/fig_objrec_difficulty.png){#fig:fig-objrec-difficulty width=40% }
...
![複数のレベルの処理により、入力全体の任意の位置でオブジェクトを認識できる不変オブジェクト認識がどのように行われるかを示す概略図。処理の各レベルは、特徴の複雑さと検出対象の空間的不変性を段階的に増加させます。これを段階的に行うことで、システムはフィーチャとその関係を適切に結合できるようになり、同時に全体的な空間的不変性を徐々に構築できるようになります。](figures/fig_invar_trans.png){#fig:fig-invar-trans width=50% }

![腹側視覚経路の領域にわたって生じる、特徴の複雑さの増大の階層を表す別の方法。 V1 には基本特徴検出器 (指向性エッジ) があります。次に、これらは V2 で線の接合部に結合され、その後 V4 でより複雑な視覚的特徴が続きます。個々の顔は IT の次のレベルで認識されます (ここでも、人々の見た目の類似度に応じて段階的に複数の顔ユニットがアクティブになります)。最後に、最も高いレベルには、ユーザーが実行する可能性のあるアクションの優れた基礎として機能する重要な機能的な「意味論的」カテゴリがあります。このような高レベルのカテゴリを開発できることは、インテリジェントな動作にとって重要です。このレベルは、IT のより前方の領域に対応します。](figures/fig_category_hierarch_dist_reps.png){#fig:fig-category-hierarch-dist-reps-6 width=100% }
...
![マカクザルの V2、V4、および IT における神経応答特性の概要。 Smax / MAX 列は、単純な視覚入力に対する応答 (Smax) が、テストされた視覚入力画像に対する最大応答 (MAX) とどのように比較されるかを示します。もう 1 つの列は視覚受容野の全体的なサイズを示しており、この領域上ではニューロンは視覚的特徴に対して比較的不変の反応を示します。 V2 では、ほぼすべてのニューロンが単純な刺激に対して最大限に反応し、受容野のサイズが最小でした。 V4 では、ニューロンの 50% のみが最大応答として単純応答を示し、受容野のサイズは V2 よりも増加しました。後部ITは両方の次元で（わずかに）増加しますが、前部ITはほぼ完全に複雑な特徴反応と大幅に大きな受容野を示します。複雑さと不変性 (受容野サイズ) のこれらの漸進的な増加は、前の図に示した不変物体認識の増分計算ソリューションによって予測されたとおりです。小畠・田中 (1994) より転載。](figures/fig_kobatake_tanaka_94_invariance.png){#fig:fig-kobatake-tanaka-94-invariance width=50% }

![V2、V4、および IT のニューロンから最大の反応を引き起こした複雑な刺激。これらのニューロンがどのような種類の複雑な特徴を検出できるかについていくつかの示唆を提供します。ほとんどの V2 ニューロンは、単純な刺激 (指向性エッジ、図示せず) に対して最大限に反応しました。小畠・田中 (1994) より転載。](figures/fig_kobatake_tanaka_94_v2_v4_it.png){#fig:fig-kobatake-tanaka-94-v2-v4-it width=50% }
...
![Waldo はどこのビジュアル検索の例。](figures/fig_wheres_waldo.jpg){#fig:fig-wheres-waldo width=50% }
...
![半側空間無視の患者による所定の対象物の描画。図面の左側が著しく無視されていることが示されています。](figures/fig_neglect_drawings.png){#fig:fig-neglect-drawings width=30% }

![半側空間無視のアーティストによるセルフポートレートの進歩。時間の経過とともに無視が徐々に改善されていることを示しています。](figures/fig_neglect_portrait.jpg){#fig:fig-neglect-portrait width=30% }

![半側空間無視のある人の線二等分タスクの結果。ここでは、無視が 2 つの異なる空間スケールで動作しているように見えることに注意してください。つまり、ラインのセット全体に対してと、個々のライン内でです。](figures/fig_neglect_line_bisect.png){#fig:fig-neglect-line-bisect width=50% }
...
![ポズナー空間キューイング タスク。空間注意効果を調査するために広く使用されています。参加者には 2 つのボックスと中央の注視十字を備えたディスプレイが表示されます。試行によっては、ボックスの 1 つがキューに出され (たとえば、線が一時的に太くなります)、その後、ボックスの 1 つにターゲットが表示されます (キャッチ トライアルではまったく表示されません)。参加者は、最初にターゲットを検出したときにキーを押すだけです。反応時間は無効なキューに比べて有効なキューの方が速く、空間的な注意が空間のその側に集められたことを示唆しています。半側空間無視の患者は、特に無効な合図の場合、空間の無視された側に現れる標的に対して速度が低下します。](figures/fig_posner_task.png){#fig:fig-posner-task width=50% }
...
![Posner 空間キューイング タスクからの典型的なデータ。キューイングのない中立トライアルと比較して、有効なトライアルでは速度が向上し、無効なトライアルでは速度が低下することを示しています。半側空間無視の患者のデータも示されており、全体的に遅い反応時間は正常な患者の反応時間に正規化されています。](figures/fig_posner_graph.png){#fig:fig-posner-graph width=30% }
...
![ポズナーの空間キューイング タスクのボックス アンド アロー モデル。彼は、空間無視の欠陥は離脱機構の損傷に起因すると示唆しています。しかし、これでは、たとえば両側頭頂部の損傷の影響は考慮されていません。](figures/fig_posner_disengage_mech.png){#fig:fig-posner-disengage-mech width=20% }

![相互作用する空間認識経路と物体認識経路により、V1 表現に対するトップダウンの影響に加えて、初期の物体認識処理に対する空間的影響という観点から、ポズナーの空間注意効果を説明できます。](figures/fig_spat_attn_lateral.png){#fig:fig-spat-attn-lateral width=30% }

ここでは、空間処理経路とオブジェクト処理経路の間の双方向の相互作用に基づいた代替アカウントを検討します ([@fig:fig-spat-attn-lateral])。この説明では、空間処理経路の半分が損傷すると、その側はネットワークの無傷の側と競合できなくなります。したがって、競合するものがあるとき（たとえば、キュ​​ーイングパラダイムにおけるキュー）、ダメージの影響が最も顕著になります。

重要なのは、これらのモデルは、*両側*頭頂部損傷の影響に関して明確な予測を行うことです。この症状を持つ患者は、視野 [@CoslettSaffran91] 内に複数の物体が存在する場合に、物体を認識する能力が著しく低下することを特徴とするバリント症候群に罹患していることが知られています。これは、混雑した視覚シーンにおける物体認識を促進する上で、空間的注意が重要な役割を果たすことを示唆しています。ポズナーの離脱モデルによれば、両側の損傷により空間の「両側」からの離脱が困難になり、空間の両側での無効な試行が遅くなるはずです。対照的に、競争ベースのモデルは逆の予測をします。つまり、損傷は空間の「両側」での競争を減少させる役割を果たし、その結果、両側で注意力の効果が減少するはずです。つまり、無効なキューの効果は実際には減少します。データは競合モデルと一致しており、Posner のモデル [@VerfaellieRapcsakHeilman90] とは一致しません。

### 空間的注意の探求

[CCNシムズ](https://compcogneuro.org/simulations) で `attn_simpl` を開き、複数の物体の知覚などの複数の空間注意タスクと Posner 空間キューイング タスクのコンテキストで相互作用する空間経路と物体経路を含むモデルを探索します。これは上記の行動データを再現しており、バリント患者の注意力の低下という観察されたパターンを正確に示しています。
